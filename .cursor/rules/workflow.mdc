...
globs: ["current_workflow/**/*.md"] # Apply primarily to workflow directory markdown files
alwaysApply: true # Guideline for documenting significant items
---
# Focused Workflow & Context Documentation (`current_workflow/`)

1.  **Mandate:** Maintain a **concise and accessible** record of **significant** development context, architectural decisions, major challenges, and strategic planning using Markdown files (`.md`) within the `current_workflow/` directory. Use Mermaid diagrams (` ```mermaid ``` `) **judiciously** where they substantially clarify complex architectures, state flows, or decision trees.
...
4.  **Guidance:**
    ...
    *   **Maintain Organization:** Use clear filenames and potentially subdirectories within `current_workflow/` (e.g., `/decisions`, `/investigations`, `/architecture`) to keep information discoverable.
