...
from hummingbot.strategy.perpetual_market_making_v2.data_types import PriceSize, Proposal
from hummingbot.strategy.perpetual_market_making_v2.perpetual_market_making_v2_order_tracker import (
    PerpetualMarketMakingOrderTrackerV2,
)
...
class PerpetualMarketMakingStrategyV2(StrategyPyBase):
    ...
    def init_params(self,
                    ...
                    stop_loss_order_filled_delay: float = 300.0,
        ...
        self._sb_order_tracker = PerpetualMarketMakingOrderTrackerV2()
        ...
        self._stop_loss_order_filled_delay = stop_loss_order_filled_delay
        ...
        self._stop_loss_orders = dict()  # Track which orders are stop loss orders
    ...
    def active_orders_df(self) -> pd.DataFrame:
        ...
        for idx in range(len(active_orders)):
    ...
    def manage_positions(self, session_positions: list[Position]):
        ...
        if proposals is not None:
            self.execute_orders_proposal(proposals, PositionAction.CLOSE, is_stop_loss=True)
    ...
    def create_base_proposal(self):
        ...
        if order_override is not None and len(order_override) > 0:
            ...
            for level in range(self._buy_levels):
                ...
                if size > 0:
                    buys.append(PriceSize(price, size))
            for level in range(self._sell_levels):
    ...
    def did_complete_buy_order(self, order_completed_event: BuyOrderCompletedEvent):
        ...
        # Check if this was a stop loss order and apply appropriate delay
        if order_id in self._stop_loss_orders:
            delay = self._stop_loss_order_filled_delay
            self._stop_loss_orders.pop(order_id, None)
            self.logger().info(f"Stop loss order {order_id} filled, applying {delay}s delay")
        else:
            delay = self._filled_order_delay

        # delay order creation by appropriate delay (in seconds)
    ...
    def is_within_tolerance(self, current_prices: list[Decimal], proposal_prices: list[Decimal]) -> bool:
        ...
        for current, proposal in zip(current_prices, proposal_prices, strict=False):
    ...
    def execute_orders_proposal(self, proposal: Proposal, position_action: PositionAction, is_stop_loss: bool = False):
        ...
        if len(proposal.buys) > 0:
            ...
            for buy in proposal.buys:
                ...
                if position_action == PositionAction.CLOSE:
                    ...
                    if is_stop_loss:
                        self._stop_loss_orders[bid_order_id] = True
                orders_created = True
        if len(proposal.sells) > 0:
            ...
            for sell in proposal.sells:
                ...
                if position_action == PositionAction.CLOSE:
                    ...
                    if is_stop_loss:
                        self._stop_loss_orders[ask_order_id] = True
                orders_created = True
        if orders_created:
            self.set_timers()
