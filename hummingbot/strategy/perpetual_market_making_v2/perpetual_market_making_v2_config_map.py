...
def maker_trading_pair_prompt():
    derivative = perpetual_market_making_v2_config_map.get("derivative").value
...
def order_amount_prompt() -> str:
    trading_pair = perpetual_market_making_v2_config_map["market"].value
...
def on_validate_price_source(value: str):
    if value != "external_market":
        perpetual_market_making_v2_config_map["price_source_derivative"].value = None
        perpetual_market_making_v2_config_map["price_source_market"].value = None
    if value != "custom_api":
        perpetual_market_making_v2_config_map["price_source_custom_api"].value = None
    else:
        perpetual_market_making_v2_config_map["price_type"].value = "custom"


def validate_price_type(value: str) -> str | None:
    error = None
    price_source = perpetual_market_making_v2_config_map.get("price_source").value
...
def price_source_market_prompt() -> str:
    external_market = perpetual_market_making_v2_config_map.get("price_source_derivative").value
    return f"Enter the token trading pair on {external_market} >>> "


def validate_price_source_derivative(value: str) -> str | None:
    if value == perpetual_market_making_v2_config_map.get("derivative").value:
...
def validate_price_source_market(value: str) -> str | None:
    market = perpetual_market_making_v2_config_map.get("price_source_derivative").value
...
perpetual_market_making_v2_config_map = {
    "strategy":
        ConfigVar(key="strategy",
                  prompt=None,
                  default="perpetual_market_making_v2"),
    "derivative":
    ...
    "order_level_amount":
        ConfigVar(key="order_level_amount",
                  ...
                  required_if=lambda: perpetual_market_making_v2_config_map.get("order_levels").value > 1,
                  type_str="decimal",
    ...
    "filled_order_delay":
        ConfigVar(key="filled_order_delay",
                  ...
                  default=60),
    "stop_loss_order_filled_delay":
        ConfigVar(key="stop_loss_order_filled_delay",
                  prompt="How long do you want to wait before placing the next order "
                         "if your STOP LOSS order gets filled (in seconds)? (Default is 300 seconds) >>> ",
                  type_str="float",
                  validator=lambda v: validate_decimal(v, min_value=0, inclusive=False),
                  default=300),
    ...
    "ask_order_optimization_depth":
        ConfigVar(key="ask_order_optimization_depth",
                  ...
                  required_if=lambda: perpetual_market_making_v2_config_map.get("order_optimization_enabled").value,
                  type_str="decimal",
    ...
    "price_type":
        ConfigVar(key="price_type",
                  ...
                  type_str="str",
                  required_if=lambda: perpetual_market_making_v2_config_map.get("price_source").value != "custom_api",
                  default="mid_price",
    ...
    "price_source_derivative":
        ConfigVar(key="price_source_derivative",
                  ...
                  required_if=lambda: perpetual_market_making_v2_config_map.get("price_source").value == "external_market",
                  type_str="str",
