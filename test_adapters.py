...
async def test_adapters():
    ...
    from cyberdelta.core.funding_rate import (
        HyperliquidAdapter, BackpackAdapter, ExchangeId
    )
    ...
    try:
        ...
        for symbol, rate_info in hyperliquid_rates.items():
            logger.info(f"  {symbol}: {rate_info.current_rate:.4f}% (volatility: {rate_info.historical_volatility:.4f}%)")
