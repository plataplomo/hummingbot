...
class BackpackExchangeTests(AbstractExchangeConnectorTests.ExchangeConnectorTests):
    ...
    def all_symbols_url(self):
        return web_utils.public_rest_url(path_url=CONSTANTS.EXCHANGE_INFO_URL, domain=self.exchange._domain)

    @property
    def latest_prices_url(self):
        url = web_utils.public_rest_url(path_url=CONSTANTS.TICKER_URL, domain=self.exchange._domain)
        url = f"{url}?symbol={self.exchange_symbol_for_tokens(self.base_asset, self.quote_asset)}"
        return url

    @property
    def network_status_url(self):
        return web_utils.public_rest_url(CONSTANTS.TIME_URL, domain=self.exchange._domain)

    @property
    def trading_rules_url(self):
        return web_utils.public_rest_url(CONSTANTS.EXCHANGE_INFO_URL, domain=self.exchange._domain)

    @property
    def order_creation_url(self):
        return web_utils.private_rest_url(CONSTANTS.ORDER_URL, domain=self.exchange._domain)

    @property
    def balance_url(self):
        return web_utils.private_rest_url(CONSTANTS.COLLATERAL_URL, domain=self.exchange._domain)
    ...
    def expected_trading_rule(self):
        min_base_size = Decimal(self.trading_rules_request_mock_response["data"][0]["filters"]["quantity"]["minQuantity"])
        min_price = Decimal(self.trading_rules_request_mock_response["data"][0]["filters"]["price"]["minPrice"])
        min_quote_size = min_base_size * min_price
        tick_size = Decimal(self.trading_rules_request_mock_response["data"][0]["filters"]["price"]["tickSize"])
        step_size = Decimal(self.trading_rules_request_mock_response["data"][0]["filters"]["quantity"]["stepSize"])
        max_order_size = Decimal(self.trading_rules_request_mock_response["data"][0]["filters"]["quantity"]["maxQuantity"])
    ...
    def create_exchange_instance(self):
        api_secret = base64.b64encode(bytes(range(32))).decode("utf-8")
    ...
    def configure_order_not_found_error_cancelation_response(
        ...
        url = web_utils.private_rest_url(CONSTANTS.ORDER_URL)
        regex_url = re.compile(f"^{url}".replace(".", r"\.").replace("?", r"\?"))
        response = {"code": CONSTANTS.ERROR_CODE_ORDER_NOT_FOUND, "msg": "Order not found."}
    ...
    def configure_erroneous_http_fill_trade_response(
            ...
            mock_api: aioresponses,
            callback: Optional[Callable] = lambda *args, **kwargs: None) -> str:
        url = web_utils.private_rest_url(path_url=CONSTANTS.FILLS_URL)
    ...
    def order_event_for_new_order_websocket_update(self, order: InFlightOrder):
        numeric_client_id = self.exchange._id_mapper.get_numeric_id(order.client_order_id)
        return {
    ...
    def test_update_time_synchronizer_failure_is_logged(self, mock_api):
        ...
        mock_api.get(regex_url, status=500, callback=lambda *args, **kwargs: request_sent_event.set())

        with self.assertRaises(IOError):
            self.async_run_with_timeout(self.exchange._update_time_synchronizer())

        self.assertTrue(self.is_logged("ERROR", "Error requesting time from Backpack server"))
    ...
    def test_update_order_fills_from_trades_triggers_filled_event(self, mock_api):
        ...
        self.assertEqual([TokenAmount(trade_fill["feeSymbol"], Decimal(trade_fill["fee"]))],
                         fill_event.trade_fee.flat_fees)

    @aioresponses()
    def test_update_order_fills_request_parameters(self, mock_api):
        self.exchange._set_current_timestamp(self.exchange.UPDATE_ORDER_STATUS_MIN_INTERVAL + 1)
    ...
    def test_update_order_fills_from_trades_with_repeated_fill_triggers_only_one_event(self, mock_api):
        ...
        mock_response = [trade_fill, trade_fill]
        mock_api.get(regex_url, body=json.dumps(mock_response))
        ...
        self.assertEqual(0.0, fill_event.trade_fee.percent)
        self.assertEqual([
            TokenAmount(trade_fill["feeSymbol"],
                        Decimal(trade_fill["fee"]))],
            fill_event.trade_fee.flat_fees)
    ...
    def test_update_order_status_when_failed(self, mock_api):
        ...
        expected_update = OrderUpdate(
            ...
            misc_updates={
                "fill_price": Decimal(order_status["price"]),
        ...
        self.assertTrue(
            self.is_logged(
                "INFO",
                f"Order {order.client_order_id} has failed. Order Update: {expected_update}")
        )
    ...
    def test_time_synchronizer_related_request_error_detection(self):
        exception = IOError("Error executing request POST https://api.backpack.exchange/api/v1/order. HTTP status is "
                            "400. Error: {'code':'EXPIRED_TIMESTAMP','msg':'Timestamp expired.'}")
        self.assertTrue(self.exchange._is_request_exception_related_to_time_synchronizer(exception))

        exception = IOError("Error executing request POST https://api.backpack.exchange/api/v1/order. HTTP status is "
                            "400. Error: {'code':'BAD_REQUEST','msg':'Timestamp is invalid.'}")
        self.assertTrue(self.exchange._is_request_exception_related_to_time_synchronizer(exception))

        exception = IOError("Error executing request POST https://api.backpack.exchange/api/v1/order. HTTP status is "
                            "400. Error: {'code':'INVALID_SIGNATURE','msg':'Signature invalid.'}")
            ...
            "data": [
                {
                    ...
                    "quoteSymbol": "HBOT",
                    "filters": {
                        "price": {"minPrice": "2", "tickSize": "0.01"},
                        "quantity": {"minQuantity": "0.5", "stepSize": "0.1"},
                    },
                }
            ]
        }
            ...
            "data": [
                {
                    ...
                    "quoteSymbol": "HBOT",
                    "filters": {
                        "price": {"minPrice": "1", "tickSize": "0.01"},
                        "quantity": {"minQuantity": "0.01", "stepSize": "0.01", "maxQuantity": "100"},
                    },
                }
            ]
        }
