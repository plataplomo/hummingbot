...
class PrioritySignalQueue:
    ...
    def __init__(self, config: Dict[str, Any], circuit_breaker_system=None):
        """
    ...
    def add_signal(self, signal: TradeSignal) -> bool:
        ...
        if len(self.signal_queue) >= self.max_queue_size:
            ...
            if len(self.signal_queue) > 0:
                ...
                if -signal.metadata["utility_score"] >= min_score:
                    logger.debug(f"Rejected signal for {signal.symbol} with score {signal.metadata['utility_score']} (lower than min {-min_score})")
                    return False
        ...
        heapq.heappush(
            self.signal_queue,
            (-signal.metadata["utility_score"], self.counter, signal)
        )
        
        logger.debug(f"Added signal for {signal.symbol} to queue with score {signal.metadata['utility_score']}")
        return True
    
    def add_from_opportunity(self, opportunity: ArbitrageOpportunity, strategy_name: str) -> Optional[TradeSignal]:
        """
        ...
        signal = TradeSignal(
            ...
            signal_type=SignalType.ENTER_LONG,  # Default to ENTER_LONG, should be adjusted based on strategy
    ...
    def peek_next_signal(self) -> Optional[TradeSignal]:
        """
    ...
    def _trim_queue(self) -> bool:
        ...
        while len(self.signal_queue) > self.max_queue_size:
            ...
            lowest_priority_idx = max(range(len(self.signal_queue)), key=lambda i: self.signal_queue[i][0])
