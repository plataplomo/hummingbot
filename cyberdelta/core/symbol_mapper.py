    ...
    def __init__(self, config: Dict) -> None:
        ...
        for exchange_id, exchange_data in exchanges_config.items():
            if not isinstance(exchange_data, dict) or "symbols" not in exchange_data:
                logger.warning(f"Skipping exchange '{exchange_id}': Missing 'symbols' configuration.")
                continue

            symbol_map = exchange_data["symbols"]
            if not isinstance(symbol_map, dict):
                logger.warning(f"Skipping exchange '{exchange_id}': 'symbols' must be a dictionary.")
                continue
