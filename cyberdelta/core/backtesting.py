...
class BacktestStrategy(ABC):
    ...
    @abstractmethod
    def update(self, current_data: Union[pd.Series, pd.DataFrame]) -> Dict[str, Any]:
        """
...
class BacktestEngine:
    ...
    def _calculate_metrics(self) -> None:
        ...
        volatility = equity_df['returns'].std() * np.sqrt(252) if len(equity_df) > 0 else 0  # Annualized
...
class StrategyAdapter(BacktestStrategy):
    ...
    def _convert_to_market_data(self, data: Union[pd.Series, pd.DataFrame]) -> MarketData:
        """
        ...
        if isinstance(data, pd.Series):
            ...
            return MarketData(
                ...
                additional_data={
                    'funding_rate': float(data.get('funding_rate', 0.0)) if 'funding_rate' in data else None
                }
            )
        else:
    ...
    def _convert_signals(self, signals: List[TradeSignal], data: Union[pd.Series, pd.DataFrame]) -> List[Dict[str, Any]]:
        """
        ...
        for signal in signals:
            ...
            if hasattr(signal, 'signal_type'):
                ...
                if signal.signal_type == SignalType.ENTER_LONG:
                    ...
                    self.positions[signal.symbol] = {'side': 'long', 'size': signal.quantity if hasattr(signal, 'quantity') else 0.1}
                    ...
                    self.positions[signal.symbol] = {'side': 'short', 'size': signal.quantity if hasattr(signal, 'quantity') else 0.1}
                ...
                if hasattr(signal, 'action'):
                    if signal.action in ['buy', 'long']:
                        ...
                        self.positions[signal.symbol] = {'side': 'long', 'size': signal.size if hasattr(signal, 'size') else 0.1}
                        ...
                        self.positions[signal.symbol] = {'side': 'short', 'size': signal.size if hasattr(signal, 'size') else 0.1}
                    elif signal.action == 'exit':
                        side = 'sell' if self.positions.get(signal.symbol, {}).get('side') == 'long' else 'buy'
                        backtest_signals.append({
                            ...
                            'price': price,
                            'size': signal.size if hasattr(signal, 'size') else 0.1,
                            'pnl': signal.expected_profit if hasattr(signal, 'expected_profit') else 0
                        })
                        if signal.symbol in self.positions:
                            del self.positions[signal.symbol]
...
def generate_synthetic_data(
    ...
    """
    if symbols is None:
        symbols = ['BTC-PERP', 'ETH-PERP', 'SOL-PERP', 'AVAX-PERP'] if data_type == 'funding_rate' else ['BTC', 'ETH']
    ...
    if data_type == 'funding_rate':
        ...
        for symbol in symbols:
            ...
            # Generate prices
            base_price = {'BTC-PERP': 30000, 'ETH-PERP': 2000, 'SOL-PERP': 100, 'AVAX-PERP': 20}.get(symbol, 100)
