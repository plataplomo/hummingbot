...
from cyberdelta.core.analytics.performance import AttributionResult
...
class AttributionAnalyzer:
    ...
    async def _calculate_period_pnl(
        ...
        # For now, use total P&L from portfolio state
        return portfolio_state.total_realized_pnl + portfolio_state.total_unrealized_pnl
    ...
    async def _attribute_by_exchange(
        ...
        for exchange_id, summary in portfolio_state.exchange_summaries.items():
            ...
            if exchange_pnl != 0:
                attribution[exchange_id] = exchange_pnl
    ...
    async def _attribute_by_symbol(
        ...
        # Without position-level data, cannot attribute by symbol
        # In production, this would query position data separately
        # For now, return empty attribution
