...
class RiskManager:
    ...
    def _calculate_kelly_size(
        ...
        try:
            ...
            logger.error(
                f"Error calculating Kelly fraction: nfd={nfd}, var={variance_risk}, avg_p={avg_price}. Error: {e}"
            )
        ...
        # === ADDED Logging ===
        logger.info(
            f"Kelly Inputs: NFD={nfd}, Vol={basis_volatility}, AvgPx={avg_price}, KellyFrac={self.kelly_fraction}"
        )
        logger.info(
            f"Kelly Calc: RawKelly={kelly:.6f}, AdjustedKelly={kelly_adjusted:.6f}, TotalCapital={total_capital:.2f}"
        )
        # === END Logging ===
    ...
    def _check_portfolio_constraints(
        ...
        if new_exposure > self.max_total_exposure:
            logger.info(
                f"Trade exceeds maximum total exposure: ${new_exposure:.2f} > ${self.max_total_exposure:.2f}"
            )
            return False
        ...
        for exchange, size in [
            ...
            if not isinstance(current_exchange_exposure, Decimal):
                logger.warning(
                    f"Current exposure for {exchange} is not Decimal: {current_exchange_exposure}. Converting."
                )
        ...
        for exchange, size in [
            ...
            logger.info(
                f"Capital Check {exchange}: Asset={collateral_asset}, Avail={available_capital_dec:.2f}, MinReq={self.min_exchange_balance:.2f}, IsInsufficient={is_insufficient}"
            )  # DETAILED DEBUG
            if is_insufficient:
                logger.warning(
                    f"No available capital on {exchange} ({collateral_asset}) (Balance: {available_capital_dec:.2f}, Min: {self.min_exchange_balance:.2f})"
                )
                return False
    ...
    def _apply_portfolio_exposure_management(
        ...
        )  # Assuming returns list of Position objects
        ...
        logger.debug(
            f"Exposure Management: Base=${base_size:.2f}, AvailTotal=${available_total_exposure:.2f}, AvailSym=${available_symbol_exposure:.2f}, AvailLongEx=${available_long_exchange:.2f}, AvailShortEx=${available_short_exchange:.2f} -> Final=${final_size:.2f}"
        )
    ...
    def _apply_portfolio_level_controls(
        ...
        try:
            ...
            logger.debug(
                "Skipping correlation limits due to missing method _apply_correlation_limits or related methods."
            )
        except Exception as e:
            ...
            try:
                ...
                for ex_name in [long_exchange, short_exchange]:
                    if self.circuit_breaker_system.is_exchange_open(ex_name):
                        ...
                        logger.warning(
                            f"Exchange circuit breaker for {ex_name} is OPEN. Applying recovery factor."
                        )
                ...
                if recovery_factor < Decimal("1.0"):
                    logger.info(
                        f"Applied circuit breaker recovery factor {recovery_factor:.2f}. New size: ${adjusted_size:.2f}"
                    )
        ...
        if self.funding_rate_validator:
            try:
                ...
                if final_factor_dec < Decimal("1.0"):
                    ...
                    logger.info(
                        f"Applied funding rate validation factor {final_factor_dec:.2f}. New size: ${adjusted_size:.2f}"
                    )
        ...
        if self.circuit_breaker_system:
            try:
                ...
                if recovery_factor < Decimal("1.0"):
                    adjusted_size *= recovery_factor
                    logger.warning(
                        f"Safety System: Circuit breaker active. Applying factor {recovery_factor:.2f}. New size: ${adjusted_size:.2f}"
                    )
            except Exception as e:
        ...
        if self.funding_rate_validator:
            try:
                ...
                if final_factor < Decimal("1.0"):
                    ...
                    logger.info(
                        f"Safety System: Applied funding validation factor {final_factor:.2f}. New size: ${adjusted_size:.2f}"
                    )
            except Exception as e:
    ...
    def _apply_volatility_adjustment(
        ...
        try:
            ...
            if vol_ratio > VOLATILITY_RATIO_THRESHOLD:
                ...
                logger.info(
                    f"High volatility detected (Ratio: {vol_ratio:.2f}). Reducing size by factor {factor:.2f}. New size: ${adjusted_size:.2f}"
                )
                return adjusted_size
            else:
                return base_size
    ...
    def _apply_drawdown_protection(
        ...
        try:
            ...
            if factor < Decimal("1.0"):
                adjusted_size = base_size * factor
                logger.info(
                    f"Drawdown protection activated (Portfolio: {portfolio_drawdown_dec:.2%}, Max Exchange: {max_exchange_drawdown:.2%}). Reducing size by factor {factor:.2f}. New size: ${adjusted_size:.2f}"
                )
                return adjusted_size
            else:
                return base_size
    ...
    def _apply_correlation_limits(self, base_size: Decimal, symbol: str) -> Decimal:
        ...
        try:
            ...
            max_allowed_correlated = total_capital * Decimal(
                str(self.max_correlated_exposure)
            )
    ...
    def _get_validation_metrics(self, exchange: str, symbol: str) -> float:
        ...
        try:
            ...
            if rmse is None or bias is None:
                logger.warning(
                    f"Incomplete validation metrics for {exchange}/{symbol}. Assuming low confidence."
                )
                return float(self.min_validation_factor)  # Return float
            ...
            logger.debug(
                f"Validation Metrics Factor for {exchange}/{symbol}: RMSE={rmse:.4f}, Bias={bias:.4f} -> Factor={final_factor:.2f}"
            )
    ...
    def validate_opportunities(
        ...
        for opportunity in opportunities:
            ...
            if sized_opp:
                ...
                logger.info(
                    f"Opportunity {opportunity.symbol} ({opportunity.long_exchange} vs {opportunity.short_exchange}) rejected during sizing/validation."
                )
