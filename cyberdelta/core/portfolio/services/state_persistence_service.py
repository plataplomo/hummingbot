...
class StatePersistenceService[T: BaseModel](BaseStateModel):
    ...
    stats: PersistenceStats = Field(
        default_factory=PersistenceStats,
    ...
    metadata: dict[str, str | int | float | bool] = Field(
        ...
        description="Service metadata with strict typing"
    )
    ...
    async def initialize(self) -> None:
        ...
        if self.is_initialized:
            logger.warning(
                "persistence_service_already_initialized",
                state_id=self.state_id
            )
            return
    ...
    def _raise_file_not_found_error(self, state_id: str, file_path: Path) -> None:
        ...
        raise TypedStatePersistenceError(
            "load", 
            state_id, 
            FileNotFoundError(f"State file not found: {file_path}")
        )
