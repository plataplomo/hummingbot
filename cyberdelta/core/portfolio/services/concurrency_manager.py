...
class ConcurrencyManager(BaseStateModel):
    ...
    max_locks_per_type: int = Field(
        default=1000,
        description="Maximum number of locks per type to prevent memory leaks"
    )
    ...
    metadata: dict[str, str | int | float | bool] = Field(
        ...
        description="Manager metadata with strict typing"
    )
    ...
    async def initialize(self) -> None:
        ...
        if self.is_initialized:
            logger.warning(
                "concurrency_manager_already_initialized",
                state_id=self.state_id
            )
            return
    ...
    async def stop(self) -> None:
        ...
        logger.info(
            "concurrency_manager_stopped",
            state_id=self.state_id,
            final_stats=self.get_lock_stats()
        )
    ...
    def _get_symbol_lock(self, symbol: str) -> asyncio.Lock:
        ...
        if symbol not in self._symbol_locks:
            ...
            logger.debug(
                "symbol_lock_created",
                symbol=symbol,
                total_symbol_locks=len(self._symbol_locks)
            )
    ...
    async def multi_exchange_lock(self, *exchange_ids: str) -> AsyncIterator[None]:
        ...
        locks: list[asyncio.Lock] = [
            self._get_exchange_lock(exchange_id) 
    ...
    @asynccontextmanager
    async def trade_processing_lock(
        self, 
        exchange_id: str, 
        symbol: str
    ) -> AsyncIterator[None]:
