...
class ComponentHealthData(BaseModel):
    ...
    errors: list[str] = Field(
        ...
        description="List of component errors"
    )
    warnings: list[str] = Field(
        default_factory=list,
        description="List of component warnings"
    )
...
class ExchangeSummaryData(BaseModel):
    ...
    account_value: Decimal = Field(
        default=Decimal(0),
        description="Total account value on exchange"
    )
    ...
    long_positions: int = Field(
        default=0,
        description="Number of long positions"
    )
    short_positions: int = Field(
        default=0,
    ...
    buy_orders: int = Field(
        default=0,
        description="Number of buy orders"
    )
    sell_orders: int = Field(
        default=0,
    ...
    spot_balances: dict[str, Decimal] = Field(
        default_factory=dict,
        description="Spot balances by asset"
    )
...
class PortfolioStateData(BaseStateModel):
    ...
    total_account_value: Decimal = Field(
        default=Decimal(0),
        description="Total account value across all exchanges"
    )
    total_collateral: Decimal = Field(
        default=Decimal(0),
        description="Total available collateral"
    )
    free_collateral: Decimal = Field(
        default=Decimal(0),
        description="Free collateral available for trading"
    )
    ...
    gross_exposure: Decimal = Field(
        default=Decimal(0),
        description="Gross exposure across all positions"
    )
    net_exposure: Decimal = Field(
        default=Decimal(0),
        description="Net exposure across all positions"
    )
    leverage: Decimal = Field(
        default=Decimal(0),
    ...
    # Risk metrics
    portfolio_var_95: Decimal = Field(
        default=Decimal(0),
    ...
    exchange_summaries: dict[str, ExchangeSummaryData] = Field(
        default_factory=dict,
        description="Summary data by exchange"
    )
    
    # Currency exposures
    currency_exposures: dict[str, Decimal] = Field(
        default_factory=dict,
        description="Exposure by currency"
    )
    
    # Component health
    component_health: dict[str, ComponentHealthData] = Field(
        default_factory=dict,
        description="Health status of portfolio components"
    )
    ...
    balances: dict[str, SpotBalance] = Field(
        default_factory=dict,
        description="Current spot balances"
    )
    trades: list[Trade] = Field(
        default_factory=_trades_factory,
        description="Recent trades"
    )
    orders: list[Order] = Field(
        default_factory=_orders_factory,
...
class TradingSessionData(BaseStateModel):
    ...
    starting_account_value: Decimal = Field(
        default=Decimal(0),
        description="Account value at session start"
    )
    starting_positions: int = Field(
        default=0,
        description="Position count at session start"
    )
    
    # Current values
    current_account_value: Decimal = Field(
        default=Decimal(0),
        description="Current account value"
    )
    current_positions: int = Field(
        default=0,
    ...
    total_trades: int = Field(
        default=0,
        description="Total trades in session"
    )
    ...
    session_realized_pnl: Decimal = Field(
        default=Decimal(0),
        description="Realized P&L for session"
    )
    session_unrealized_pnl: Decimal = Field(
        default=Decimal(0),
        description="Unrealized P&L for session"
    )
    max_profit: Decimal = Field(
        default=Decimal(0),
        description="Maximum profit reached"
    )
    max_loss: Decimal = Field(
        default=Decimal(0),
        description="Maximum loss reached"
    )
    
    # Risk metrics
    max_exposure: Decimal = Field(
        default=Decimal(0),
        description="Maximum exposure reached"
    )
    ...
    max_drawdown: Decimal = Field(
        default=Decimal(0),
        description="Maximum drawdown in session"
    )
    
    # Fee tracking
    total_fees_paid: Decimal = Field(
        default=Decimal(0),
        description="Total fees paid in session"
    )
    fees_by_exchange: dict[str, Decimal] = Field(
        default_factory=dict,
        description="Fees paid by exchange"
    )
...
class PortfolioState(PortfolioStateData):
    ...
    def _validate_cross_consistency(self) -> ValidationResult:
        ...
        if self.exchange_summaries:
            ...
            if value_diff > Decimal(1000):  # $1000 tolerance
                result.add_warning(
                    "Mismatch between portfolio total and exchange summaries"
                )
            ...
            if exchange_total_positions != self.active_positions:
                result.add_warning(
                    "Mismatch between portfolio and exchange position counts"
                )
    ...
    def get_unhealthy_components(self) -> list[str]:
        ...
        """
        return [
            component for component, health in self.component_health.items()
