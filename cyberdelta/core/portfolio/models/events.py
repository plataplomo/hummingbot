...
class EventMetadata(BaseModel):
    ...
    custom_tags: dict[str, str | int | float | bool] = Field(
        default_factory=dict, description="Custom tags for event categorization"
    )
...
class StateChangeEvent(BaseStateModel):
    ...
    event_type: Literal["state_change"] = Field(
        default="state_change", description="Event type discriminator"
    )
    ...
    old_state: BaseStateModel | None = Field(default=None, description="Previous state (optional)")
    new_state: BaseStateModel | None = Field(default=None, description="New state (optional)")

    # Change details
    changed_fields: list[str] = Field(
        default_factory=list, description="List of fields that changed"
    )
    change_reason: str | None = Field(default=None, description="Reason for the state change")
    ...
    occurred_at: datetime = Field(
        default_factory=lambda: datetime.now(UTC), description="When the state change occurred"
    )
...
class ErrorEvent(BaseStateModel):
    ...
    # Discriminator for union types
    event_type: Literal["error"] = Field(default="error", description="Event type discriminator")
    ...
    error_category: Literal[
        ...
        "system",
        "unknown",
    ] = Field(default="unknown", description="Category of error")
    ...
    # Context information
    entity_type: str | None = Field(default=None, description="Type of entity related to the error")
    entity_id: str | None = Field(default=None, description="ID of entity related to the error")
    operation: str | None = Field(default=None, description="Operation that caused the error")

    # Exception details
    exception_type: str | None = Field(default=None, description="Type of exception that occurred")
    stack_trace: str | None = Field(default=None, description="Stack trace (optional for security)")

    # Recovery information
    is_recoverable: bool = Field(default=True, description="Whether the error is recoverable")
    recovery_suggestion: str | None = Field(default=None, description="Suggested recovery action")
...
class ValidationEvent(BaseStateModel):
    ...
    event_type: Literal["validation"] = Field(
        default="validation", description="Event type discriminator"
    )
    ...
    validation_duration_ms: float | None = Field(
        default=None, description="Validation duration in milliseconds"
    )
    ...
    # Timing
    occurred_at: datetime = Field(
        default_factory=lambda: datetime.now(UTC), description="When validation occurred"
    )
...
class MetricsEvent(BaseStateModel):
    ...
    event_type: Literal["metrics"] = Field(
        default="metrics", description="Event type discriminator"
    )

    # Metrics classification
    metric_category: Literal["performance", "business", "technical", "usage", "financial"] = Field(
        ..., description="Category of metrics"
    )

    # Metrics data
    metrics: dict[str, float | int] = Field(..., description="Metrics data with numeric values")

    # Context
    component: str = Field(..., description="Component that generated the metrics")
    entity_type: str | None = Field(
        default=None, description="Type of entity the metrics relate to"
    )
    entity_id: str | None = Field(default=None, description="ID of entity the metrics relate to")

    # Aggregation info
    aggregation_window: str | None = Field(
        default=None, description="Time window for aggregated metrics (e.g., '1h', '1d')"
    )
    sample_count: int | None = Field(
        default=None, description="Number of samples in aggregated metrics"
    )
    ...
    # Timing
    occurred_at: datetime = Field(
        default_factory=lambda: datetime.now(UTC), description="When metrics were collected"
    )
    ...
    def get_summary(self) -> str:
        ...
        return (
            f"{self.metric_category.title()} metrics from {self.component} ({metric_count} metrics)"
        )
...
def create_state_change_event(
    ...
    **metadata_kwargs: str | float | bool,
    ...
    """
    metadata = EventMetadata(source_component=source_component, custom_tags=metadata_kwargs)

    return StateChangeEvent(
        state_id=(
            state_id or f"event_{entity_type}_{entity_id}_{int(datetime.now(UTC).timestamp())}"
        ),
...
def create_validation_event(
    ...
    **metadata_kwargs: str | float | bool,
    ...
    return ValidationEvent(
        state_id=(
            state_id
            or f"validation_{validation_type}_{entity_id}_{int(datetime.now(UTC).timestamp())}"
        ),
...
def create_metrics_event(
    metric_category: Literal["performance", "business", "technical", "usage", "financial"],
    ...
    **metadata_kwargs: str | float | bool,
    ...
    """
    metadata = EventMetadata(source_component=component, custom_tags=metadata_kwargs)

    return MetricsEvent(
        state_id=state_id
        or (f"metrics_{metric_category}_{component}_{int(datetime.now(UTC).timestamp())}"),
