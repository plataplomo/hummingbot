...
class RealTimeDashboard:
    ...
    def setup_layout(self):
        ...
        self.app.layout = dbc.Container(
            [
                dbc.Row([
                    dbc.Col([
                        html.H1("CyberDeltaEngine Dashboard", className="text-center my-4"),
                        html.Div(id="last-update-time", className="text-center text-muted mb-4"),
                    ], width=12)
                ]),
                ...
                dbc.Row([
                    dbc.Col([
                        dbc.Card([
                            ...
                            dbc.CardBody([
                                dcc.Graph(id="performance-overview", style={"height": "400px"}),
                            ]),
                        ]),
                ...
                dbc.Row([
                    dbc.Col([
                        dbc.Card([
                            ...
                            dbc.CardBody([
                                dcc.Graph(id="trade-analysis", style={"height": "400px"}),
                            ]),
                        ]),
                    ], width=6, className="mt-4"),
                    ...
                    dbc.Col([
                        dbc.Card([
                            ...
                            dbc.CardBody([
                                dcc.Graph(id="funding-rate-heatmap", style={"height": "400px"}),
                            ]),
                        ]),
                    ], width=6, className="mt-4"),
                ]),
    ...
    def setup_callbacks(self):
        ...
        def update_strategy_options(n_intervals):
            strategies = self.performance_tracker.get_strategy_names()
            options = [{"label": s, "value": s} for s in strategies]
            
            # Select all strategies by default if this is the first load
            value = [s for s in strategies] if n_intervals == 0 and strategies else []
        ...
        def update_time_display(n_intervals):
            self.last_update_time = datetime.now()
        ...
        def update_performance_overview(selected_strategies, time_range, n_intervals):
        ...
        def update_drawdown_chart(selected_strategies, time_range, n_intervals):
            if not selected_strategies:
                return go.Figure().update_layout(template="plotly_dark")
        ...
        def update_pnl_distribution(selected_strategies, time_range, n_intervals):
            if not selected_strategies:
                return go.Figure().update_layout(template="plotly_dark")
        ...
        def update_trade_analysis(selected_strategies, time_range, n_intervals):
            if not selected_strategies:
                return go.Figure().update_layout(template="plotly_dark")
        ...
        def update_funding_rate_heatmap(time_range, n_intervals):
        ...
        def update_performance_metrics(selected_strategies, time_range, n_intervals):
            if not selected_strategies:
            ...
            for strategy in selected_strategies:
                if strategy in returns_data.columns:
                    ...
                    strategy_trades = trade_data[trade_data['strategy'] == strategy] if not trade_data.empty else None
                    ...
                    metrics_rows.append(
                        html.Tr([
                            ...
                            html.Td(f"{metrics.get('win_rate', 'N/A'):.2f}%" if isinstance(metrics.get('win_rate'), (int, float)) else "N/A"),
                            html.Td(f"{metrics.get('profit_factor', 'N/A'):.2f}" if isinstance(metrics.get('profit_factor'), (int, float)) else "N/A"),
                        ])
                    )
    ...
    def start(self, use_threading: bool = True):
        """
