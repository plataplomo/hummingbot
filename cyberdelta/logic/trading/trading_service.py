...
class TradingService(HealthCheckable):
    ...
    async def execute_signal(self, signal: TradeSignal) -> Optional[Trade]:
        ...
        try:
            ...
            logger.info(
                ...
                executed_quantity=float(trade.executed_quantity) if trade.executed_quantity else None
            )
    ...
    async def _publish_trade_events(self, trade: Trade, original_signal: TradeSignal) -> None:
        ...
        try:
            ...
            await self._event_bus.publish("signal_completed", {
    ...
    async def _publish_execution_error(self, signal: TradeSignal, error: Exception) -> None:
        ...
        """
        try:
            await self._event_bus.publish("signal_execution_failed", {
                "signal_id": signal.signal_id,
                "symbol": signal.symbol.value,
                "exchange": signal.exchange.value if hasattr(signal.exchange, 'value') else str(signal.exchange),
                "error": str(error),
    ...
    async def get_execution_status(self) -> dict:
        ...
        return {
            "configuration": {
                ...
                "retry_enabled": self._execution_config.retry_enabled,
                "max_retry_attempts": self._execution_config.max_retry_attempts if self._execution_config.retry_enabled else 0
            },
            "engine_status": await self._execution_engine.get_status() if hasattr(self._execution_engine, 'get_status') else "unknown"
        }
    ...
    async def check_health(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": self._success_count / self._signal_count if self._signal_count > 0 else 0.0
        }
