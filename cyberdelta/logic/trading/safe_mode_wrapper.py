...
class SafeModeWrapper:
    ...
    def __init__(
        ...
        if self._paper_config:
            self._initial_balance = getattr(
                self._paper_config,
                'initial_balance',
                Decimal("10000")
            )
            self._fill_probability = getattr(
                self._paper_config,
                'fill_probability',
                0.95
            )
        ...
        if exchange_config:
            self._maker_fee_rate = getattr(
                exchange_config,
                'maker_fee_rate',
                Decimal("0.0002")
            )
            self._taker_fee_rate = getattr(
                exchange_config,
                'taker_fee_rate',
                Decimal("0.0005")
            )
    ...
    async def _simulate_fill(self, order: Order, immediate: bool = False) -> None:
        ...
        # Apply slippage
        slippage_factor = Decimal(str(random.uniform(
            float(-self._slippage_range),
            float(self._slippage_range)
        )))
    ...
    async def _simulate_limit_fill(self, order: Order) -> None:
        ...
        fill_delay = getattr(
            self._paper_config,
            'limit_fill_delay_seconds',
            5.0
        ) if self._paper_config else 5.0
    ...
    def get_simulated_stats(self) -> Dict[str, Any]:
        ...
        filled_orders = sum(
            1 for o in self._simulated_orders.values()
            if o.status == OrderStatus.FILLED
        )
        
        total_volume = sum(
            f.quantity * f.price for f in self._simulated_fills
        )
