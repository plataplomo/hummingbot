...
class OrderValidator:
    ...
    async def validate_order(self, order: Order) -> List[str]:
        ...
        try:
            ...
            logger.error(
                "order_validation_error", order_id=order.order_id, error=str(e), exc_info=True
            )
            raise
    ...
    async def _validate_exchange_constraints(
        self, order: Order, exchange_config: object
    ) -> List[str]:
    ...
    async def _validate_market_conditions(self, order: Order, exchange_config: object) -> List[str]:
        ...
        try:
            ...
            if (
                hasattr(exchange_config, "max_price_deviation_pct")
                and order.price
    ...
    def _validate_order_structure(self, order: Order) -> List[str]:
        ...
        if not isinstance(order.exchange, ExchangeName):
            violations.append(
                f"Order exchange must be ExchangeName type, got {type(order.exchange)}"
            )
    ...
    async def validate_order_modification(
        ...
        if hasattr(exchange_config, "max_quantity_change_pct"):
            ...
            quantity_change = (
                abs(modified_order.quantity - original_order.quantity)
