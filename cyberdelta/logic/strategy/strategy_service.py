...
class StrategyService:
    ...
    async def _execution_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                # Run each strategy
                signals_generated = await self._run_all_strategies(market_snapshot, portfolio_state)
    ...
    async def _process_strategy_signal(self, signal: TradeSignal, strategy_name: str) -> bool:
        ...
        Returns:
            True if signal was successfully processed, False otherwise
        """
        # Validate signal has required fields
        self._validate_generated_signal(signal, strategy_name)
    ...
    async def _run_all_strategies(
        self, market_snapshot: MarketSnapshot, portfolio_state: PortfolioState
    ) -> int:
        """Run all registered strategies and count successful signals.
        ...
        Returns:
            Number of successfully generated signals
        """
        signals_generated = 0
        for name, strategy in self._strategies.items():
            try:
                ...
                if signal:
                    success = await self._process_strategy_signal(signal, name)
                    if success:
                        signals_generated += 1
