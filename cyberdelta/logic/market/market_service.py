        ...
        for exchange_name, exchange_config in self._enabled_exchanges.items():
            try:
                ...
                await asyncio.wait_for(
                    self._initialize_exchange_connection(api_client, exchange_name),
                    timeout=timeout
                )
                
                logger.info(
                    "exchange_connection_initialized",
                    exchange=exchange_name,
    ...
    async def get_ticker(
        ...
        try:
            ...
            if not exchange_config:
                logger.warning(
                    "ticker_fetch_exchange_disabled",
                    symbol=symbol.value,
                    exchange=exchange.value
                )
                return None
            ...
            ticker = await asyncio.wait_for(
                self._fetch_ticker_from_api(api_client, symbol, exchange),
                timeout=timeout
            )
    ...
    async def _fetch_exchange_data(
        ...
        try:
            ...
            tickers_task = asyncio.create_task(
                self._fetch_all_tickers(api_client, exchange_name)
            )
            ...
            tickers, order_books = await asyncio.wait_for(
                asyncio.gather(tickers_task, order_books_task),
                timeout=timeout
            )
