...
class RiskService:
    ...
    async def assess_signal(self, signal: TradeSignal) -> RiskAssessment:
        ...
        try:
            ...
            position = await self._portfolio_service.get_position(
                signal.symbol,
                ExchangeName(signal.exchange)
                if isinstance(signal.exchange, str)
                else signal.exchange,
            )
            ...
            logger.error(
                "risk_assessment_failed", signal_id=signal.signal_id, error=str(e), exc_info=True
            )
            raise
    ...
    async def calculate_position_size(
        ...
        position_size = PositionSize(
            ...
            percent_of_equity=position_value / total_equity * 100
            if total_equity > 0
            else Decimal("0"),
        )
    ...
    def _calculate_exposure(
        self, position: Optional[object], signal_price: Optional[Decimal]
    ) -> Decimal:
    ...
    async def update_drawdown_monitoring(self, portfolio_value: Optional[Decimal] = None) -> None:
        ...
        try:
            ...
            logger.error("drawdown_monitoring_update_failed", error=str(e), exc_info=True)
