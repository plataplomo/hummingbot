...
class DrawdownMonitor:
    ...
    async def update_portfolio_value(self, current_value: Optional[Decimal] = None) -> None:
        ...
        try:
            ...
            self._value_history = [
                entry for entry in self._value_history
        ...
        if self._current_drawdown_pct > self._max_drawdown_pct:
            ...
            if self._drawdown_violated:
                ...
                logger.info(
                    ...
                    ).total_seconds() if self._violation_timestamp else 0
                )
    ...
    def get_drawdown_status(self) -> Dict[str, object]:
        ...
        return {
            ...
            "violation_timestamp": (
                self._violation_timestamp.isoformat() 
                if self._violation_timestamp else None
            ),
            "peak_value": float(self._peak_value),
            "peak_timestamp": (
                self._peak_timestamp.isoformat() 
                if self._peak_timestamp else None
            ),
            "trough_value": float(self._trough_value),
            "trough_timestamp": (
                self._trough_timestamp.isoformat() 
                if self._trough_timestamp else None
            ),
            "history_entries": len(self._value_history),
            "lookback_days": self._lookback_days,
            "configuration": {
    ...
    async def check_drawdown_limits(self) -> List[str]:
        ...
        if hasattr(self._global_risk_config, 'drawdown_warning_pct'):
            ...
            if (self._current_drawdown_pct > warning_threshold and 
                not self._drawdown_violated):
                violations.append(
