...
class PerformanceTracker:
    ...
    async def _calculate_total_pnl(
        self,
        period_start: datetime,
        period_end: datetime
    ) -> Decimal:
        ...
        for trade in trades:
            trade_pnl = trade.quantity * (trade.price if trade.side.value == "sell" else -trade.price)
    ...
    async def _calculate_total_return(
        self,
        total_pnl: Decimal,
        period_start: datetime
    ) -> Decimal:
        ...
        if not starting_equity or starting_equity == Decimal("0"):
            logger.warning(
                "cannot_calculate_return_zero_equity",
                period_start=period_start.isoformat()
            )
            return Decimal("0")
    ...
    async def _calculate_trading_statistics(
        ...
        for trade in trades:
            ...
            trade_pnl = trade.quantity * trade.price * (Decimal("1") if trade.side.value == "sell" else Decimal("-1"))
        ...
        win_rate = (Decimal(num_winners) / Decimal(total_trades) * Decimal("100")) if total_trades > 0 else Decimal("0")
    ...
    async def _get_trades_in_period(
        ...
        return [
            t for t in self._trade_history
            if period_start <= t.executed_at <= period_end
        ]
    ...
    def _calculate_peak_to_trough_drawdown(
        ...
        for timestamp, equity in equity_curve:
            if equity > peak:
                ...
                drawdown = (peak - equity) / peak * Decimal("100") if peak > Decimal("0") else Decimal("0")
        ...
        if equity_curve:
            ...
            current_drawdown = (peak - current_equity) / peak * Decimal("100") if peak > Decimal("0") else Decimal("0")
    ...
    async def _calculate_unrealized_pnl(self, portfolio_state) -> Decimal:
        ...
        return portfolio_state.total_unrealized_pnl if hasattr(portfolio_state, "total_unrealized_pnl") else Decimal("0")
