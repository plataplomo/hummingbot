...
class HealthMonitor:
    ...
    async def check_service_health(self, service_name: str) -> HealthCheck:
        ...
        try:
            ...
            return HealthCheck(
                ...
                thresholds_used={"timeout_sec": float(self._monitoring_config.health_check_timeout_seconds)}
            )
    ...
    async def get_system_health(self) -> SystemHealthReport:
        ...
        for service_name in self._services.keys():
            try:
                ...
                logger.error(
                    "system_health_check_service_failed",
                    service_name=service_name,
                    error=str(e)
                )
                # Create a failed health check
                service_checks.append(HealthCheck(
                    service_name=service_name,
        ...
        logger.info(
            ...
            healthy_services=sum(1 for check in service_checks if check.status == HealthStatus.HEALTHY),
    ...
    def _determine_overall_status(self, service_checks: List[HealthCheck]) -> HealthStatus:
        ...
        if critical_count >= total_services * critical_threshold:
            ...
            return HealthStatus.UNHEALTHY
        elif (critical_count + unhealthy_count + degraded_count) >= total_services * degraded_threshold:
            return HealthStatus.DEGRADED
        else:
            return HealthStatus.HEALTHY
    ...
    def _check_alerts(
        ...
        critical_services = [
            check.service_name for check in service_checks 
        ...
        total_errors = sum(
            check.metrics.get("error_count", 0) if check.metrics else 0 
            for check in service_checks
        )
        total_requests = sum(
            (check.metrics.get("error_count", 0) + check.metrics.get("success_count", 0)) 
            if check.metrics else 0 
            for check in service_checks
        )
