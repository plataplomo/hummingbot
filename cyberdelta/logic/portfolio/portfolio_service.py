...
class PortfolioService(HealthCheckable):
    ...
    async def initialize(self) -> None:
        ...
        async with self._state_lock:
            ...
            try:
                ...
                logger.error(
                    "portfolio_initialization_failed",
                    error=str(e),
                    exc_info=True
                )
                raise
    ...
    async def get_total_equity_usd(self) -> Decimal:
        ...
        async with self._state_lock:
            ...
            logger.debug(
                "total_equity_calculation_deferred",
                reason="market_data_service_not_available"
            )
            return Decimal("0")
    ...
    async def get_exchange_balances(
        ...
        async with self._state_lock:
            if self._cached_state is None:
                logger.warning(
                    "exchange_balances_request_before_initialization",
                    exchange=exchange.value
                )
                return {}
    ...
    async def get_exchange_positions(
        ...
        async with self._state_lock:
            if self._cached_state is None:
                logger.warning(
                    "exchange_positions_request_before_initialization",
                    exchange=exchange.value
                )
                return {}
    ...
    async def _update_position_from_trade(self, trade: Trade) -> None:
        ...
        if current_position is None:
            ...
            quantity_change = (
                trade.quantity if trade.side == OrderSide.BUY 
                else -trade.quantity
            )
    ...
    async def _update_balance_from_trade(self, trade: Trade) -> None:
        ...
        balance_change = (
            -trade_cost if trade.side == OrderSide.BUY 
            else trade_cost
        )
        ...
        async with self._state_lock:
            try:
                ...
                if loaded_state is not None:
                    old_timestamp = (
                        self._cached_state.timestamp if self._cached_state 
                        else None
                    )
    ...
    async def update_position_directly(
        ...
        symbol: Symbol, 
        exchange: ExchangeName, 
        new_position: Optional[DerivativePosition]
    ) -> None:
    ...
    async def reconcile_with_exchanges(self) -> None:
        ...
        if not self._api_clients:
            logger.warning(
                ...
                reason="no_api_clients_configured"
            )
            return
        ...
        for exchange_name, api_client in self._api_clients.items():
            try:
                ...
                try:
                    ...
                    logger.error(
                        "reconciliation_invalid_exchange",
                ...
                if not exchange_config or not exchange_config.enabled:
                    logger.debug(
                        "reconciliation_skipped_disabled_exchange",
                        exchange=exchange_name
                    )
                    continue
    ...
    async def _reconcile_exchange_balances(
        ...
        try:
            ...
            exchange_balances = await asyncio.wait_for(
                self._fetch_exchange_balances(api_client, exchange),
            ...
            for asset_name, exchange_balance in exchange_balances.items():
                ...
                if cached_balance is None:
                    ...
                    difference = abs(exchange_balance.total_quantity - cached_balance.total_quantity)
    ...
    async def _reconcile_exchange_positions(
        ...
        try:
            ...
            exchange_positions = await asyncio.wait_for(
                self._fetch_exchange_positions(api_client, exchange),
                timeout=float(timeout)
            )
            ...
            for symbol_name, cached_position in cached_positions.items():
                if symbol_name not in exchange_positions:
                    if abs(cached_position.size) > tolerance:
                        ...
                        await self.update_position_directly(
                            Symbol(symbol_name), exchange, None
                        )
    ...
    async def _fetch_exchange_positions(
        ...
        try:
            ...
            if hasattr(api_client, 'get_account_positions'):
                ...
                logger.debug(
                    "api_client_no_position_support",
                    exchange=exchange.value
                )
        ...
        async with self._state_lock:
            ...
            try:
                if pnl_method == "mark_to_market":
                    pnl_report = await self._calculate_mark_to_market_pnl(include_fees, base_currency)
                elif pnl_method == "realized_only":
                    pnl_report = await self._calculate_realized_pnl_only(include_fees, base_currency)
                elif pnl_method == "comprehensive":
                    pnl_report = await self._calculate_comprehensive_pnl(include_fees, base_currency)
                else:
    ...
    async def _calculate_comprehensive_pnl(
        ...
        if performance_config.calculate_time_weighted_returns:
            time_weighted_return = await self._calculate_time_weighted_return(performance_period_days)
    ...
    async def _calculate_realized_pnl_from_positions(self) -> Decimal:
        ...
        logger.debug(
            ...
            reason="trade_history_integration_pending"
        )
    ...
    async def check_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            state_age_seconds = 0
            if self._cached_state:
                state_age_seconds = (datetime.now(UTC) - self._cached_state.timestamp).total_seconds()
            ...
            success_rate = (
                self._success_count / self._operation_count 
    ...
    async def get_performance_metrics(self, period_days: Optional[int] = None) -> Optional[Dict[str, object]]:
        ...
        if self._performance_tracker is None:
            logger.debug(
                "performance_metrics_not_available",
                reason="performance_tracking_not_enabled"
            )
            return None
        ...
        try:
            ...
            logger.error(
                "performance_metrics_calculation_failed",
                error=str(e),
                exc_info=True
            )
            return None
