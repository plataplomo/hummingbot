...
class PerformanceMetricsCalculator:
    ...
    def calculate_sortino_ratio(
        ...
        downside_deviation = np.sqrt((downside_returns**2).sum() / len(returns)) * np.sqrt(
            periods_per_year
        )
    ...
    def calculate_calmar_ratio(returns: pd.Series, periods_per_year: int = 252) -> float:
        ...
        max_drawdown = (
            PerformanceMetricsCalculator.calculate_max_drawdown(returns) / 100
        )  # Convert to decimal
            ...
            metrics.update(
                {
                    ...
                    if not trades[trades["pnl"] > 0].empty
                    else 0,
...
if __name__ == "__main__":
    ...
    metrics = calculator.calculate_all_metrics(
        returns_data["Strategy1"], trade_data[trade_data["strategy"] == "Strategy1"]
    )
    print(metrics)
