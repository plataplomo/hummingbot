...
class FundingRateArbitrageStrategy(Strategy):
    ...
    def __init__(self, 
        ...
        self.min_profit_threshold = self.get_param('min_profit_threshold', 5.0)  # $5 minimum expected profit
        self.risk_aversion = self.get_param('risk_aversion', 1.0)  # Risk aversion parameter for utility function
        self.rebalance_threshold = self.get_param('rebalance_threshold', 0.05)  # 5% threshold for rebalancing
        ...
        self.check_interval = self.get_param('check_interval', 600)  # 10 minutes by default
        
        # Exchange mapping
        self.perp_exchange = self.get_param('perp_exchange', 'hyperliquid')
        self.spot_exchange = self.get_param('spot_exchange', 'backpack')
        
        # Market mapping (perp to spot)
        # e.g., "BTC-PERP" -> "BTC_USDC"
        self.symbol_mapping = self.get_param('symbol_mapping', {})
    ...
    async def _check_opportunity(self) -> Optional[ArbitrageOpportunity]:
        ...
        position_size = 1000.0  # Default position size, will be adjusted by risk manager
        ...
        perp_fee_rate = self.get_param(f'{self.perp_exchange}_fee_rate', 0.0005)
        spot_fee_rate = self.get_param(f'{self.spot_exchange}_fee_rate', 0.0005)
        
        # Calculate total costs
        total_costs = (position_size * (perp_slippage + spot_slippage + perp_fee_rate + spot_fee_rate))
        
        # Calculate expected profit (convert funding rate from percentage to decimal)
        expected_profit = (position_size * (nfd / 100)) - total_costs
        
        # Skip if expected profit is below threshold
        if expected_profit < self.min_profit_threshold:
            logger.debug(f"Expected profit (${expected_profit:.2f}) below threshold (${self.min_profit_threshold:.2f})")
            return None
        ...
        opportunity = ArbitrageOpportunity(
            ...
            long_funding_rate=nfd if perp_side == "LONG" else 0,
            short_funding_rate=nfd if perp_side == "SHORT" else 0,
    ...
    def _calculate_basis_volatility(self, symbol: str) -> float:
        """
        ...
        if symbol not in self.historical_basis or len(self.historical_basis[symbol]) < 2:
            ...
            return 0.01  # Default volatility of 1%
    ...
    def _estimate_slippage(self, symbol: str, size: float, exchange: str) -> float:
        """
        ...
        # Scale slippage based on size
        # Assuming $10,000 as reference size for 0.01% slippage
        slippage_scaling = (size / 10000) ** 0.5
    ...
    def _generate_rebalance_signal(self) -> TradeSignal:
        ...
        perp_side = "LONG" if (perp_position.quantity > 0 and perp_adjustment > 0) or \
                             (perp_position.quantity < 0 and perp_adjustment < 0) else "SHORT"
        
        spot_side = "LONG" if (spot_position.quantity > 0 and spot_adjustment > 0) or \
                             (spot_position.quantity < 0 and spot_adjustment < 0) else "SHORT"
