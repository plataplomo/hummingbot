...
All parsing errors will include the field name in their messages if provided, greatly improving error traceability.
"""
...
def parse_datetime_utc(value: Any, field_name: str = "") -> datetime | None:
    """
    ...
    Args:
        ...
        field_name: (Optional) The name of the field being parsed. If provided, it will be included in any error messages for better traceability.
    Returns:
        A timezone-aware UTC datetime object, or None if value is None and allowed.
    Raises:
        ValueError: If the value cannot be parsed as a datetime. The error message will include the field name if provided.
    """
...
def parse_decimal_value(
    value: Any, allow_none: bool = True, field_name: str = ""
) -> Decimal | None:
    """
    ...
    Raises:
        ValueError: If the value cannot be parsed as a Decimal. The error message will include the field name if provided.
    """
    prefix = f"{field_name}: " if field_name else ""
    if value is None:
        if allow_none:
