...
from cyberdelta.validation.circuit_breaker import (
    BreakerState, CircuitBreaker, CircuitBreakerSystem,
    VolatilityBreaker, DrawdownBreaker, APIErrorBreaker, LiquidityBreaker
)
...
class TestCircuitBreakerBase:
    ...
    def test_trip(self):
        ...
        assert breaker.trip_reason == "Test reason"
        assert breaker.trip_count == 1
        
        # Trip again to test counter
        breaker.trip("Another reason")
        
        assert breaker.trip_count == 2
        assert breaker.trip_reason == "Another reason"
    ...
    def test_allow_operation(self):
        ...
        # Wait for cooldown
        breaker.trip_time = datetime.now() - timedelta(seconds=2)
        assert breaker.allow_operation() is True
...
class TestAPIErrorBreaker:
    ...
    def test_record_error_above_threshold(self):
        ...
        assert breaker.state == BreakerState.OPEN
        assert "3 errors" in breaker.trip_reason
...
class TestCircuitBreakerSystem:
    ...
    def config(self):
        ...
        config.get.side_effect = lambda key, default=None: {
            ...
            'exchanges.backpack.symbols': ['BTC', 'SOL'],
            'validation.circuit_breaker.exchanges.hyperliquid.enabled': True,
            'validation.circuit_breaker.exchanges.hyperliquid.api_errors.enabled': True,
            'validation.circuit_breaker.exchanges.hyperliquid.api_errors.threshold': 3,
            'validation.circuit_breaker.exchanges.hyperliquid.volatility.enabled': True,
            'validation.circuit_breaker.exchanges.hyperliquid.drawdown.enabled': True,
            'validation.circuit_breaker.exchanges.hyperliquid.liquidity.enabled': True,
            'validation.circuit_breaker.exchanges.backpack.enabled': True
        }.get(key, default)
    ...
    def breaker_system(self, config):
        """Create a CircuitBreakerSystem instance for testing."""
    ...
    def test_init_and_load_config(self, breaker_system):
        ...
        assert isinstance(breaker_system.exchange_breakers['hyperliquid']['api_errors'], APIErrorBreaker)
        assert isinstance(breaker_system.exchange_breakers['hyperliquid']['BTC_volatility'], VolatilityBreaker)
        assert isinstance(breaker_system.exchange_breakers['hyperliquid']['drawdown'], DrawdownBreaker)
    ...
    def test_update_price(self, breaker_system):
        ...
        # Get the breaker
        vol_breaker = breaker_system.get_exchange_breaker('hyperliquid', 'BTC_volatility')
    ...
    def test_update_liquidity(self, breaker_system):
        ...
        # Get the breaker
        liq_breaker = breaker_system.get_exchange_breaker('hyperliquid', 'BTC_liquidity')
    ...
    def test_reset_breaker(self, breaker_system):
        ...
        # Reset by name
        success = breaker_system.reset_breaker('hyperliquid_api_errors')
        
        assert success is True
        assert api_breaker.state == BreakerState.CLOSED
        
        # Try to reset nonexistent breaker
        assert breaker_system.reset_breaker('nonexistent') is False
    ...
    def test_get_status(self, breaker_system):
        ...
        assert status['exchange_breakers']['hyperliquid']['api_errors']['state'] == "OPEN"
