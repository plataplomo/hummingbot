...
class BacktestResultsHandler:
    ...
    def calculate_metrics(self) -> dict[str, Any]:
        ...
        annualized_return = (
            1 + returns.mean()
        ) ** 252 - 1  # Assuming daily returns (adjust if needed)
        ...
        # --- Trade-Based Metrics ---
        if (
            self.trades_df is not None
            and not self.trades_df.empty
            and "pnl" in self.trades_df.columns
        ):
            ...
            if len(trades) > 0:
                ...
                self.metrics["win_rate_pct"] = (
                    (len(winning_trades) / len(trades)) * 100 if len(trades) > 0 else 0
                )
                ...
                self.metrics["avg_win_pnl"] = (
                    winning_trades["pnl"].mean() if not winning_trades.empty else 0
                )
                self.metrics["avg_loss_pnl"] = (
                    losing_trades["pnl"].mean() if not losing_trades.empty else 0
                )
                ...
                if "entry_time" in trades.columns and "exit_time" in trades.columns:
                    ...
                    if not trades_clean.empty:
                        ...
                        if (
                            pd.notna(avg_period)
                            and hasattr(avg_period, "total_seconds")
                            and callable(getattr(avg_period, "total_seconds", None))
                        ):
