...
class PositionSize(BaseModel):
    ...
    quantity: Decimal = Field(
        description="Quantity to trade in base asset units"
    )
...
class RiskAssessment(BaseModel):
    ...
    signal_id: str = Field(
        description="ID of the signal being assessed"
    )
    
    approved: bool = Field(
        description="Whether the signal is approved for execution"
    )
    ...
    current_exposure: Decimal = Field(
        description="Current total exposure in USD before this trade"
    )
    ...
    max_loss_usd: Optional[Decimal] = Field(
        default=None,
        description="Maximum potential loss in USD if stop loss is set"
    )
    
    risk_reward_ratio: Optional[Decimal] = Field(
        default=None,
        description="Risk/reward ratio if take profit and stop loss are set"
    )
