...
class BackpackRawFill(BaseModel):
    ...
    def validate_optional_string(cls, v: object | None, info: ValidationInfo) -> str | None:
        """Validate optional string (type, length, UTF-8), allow empty string at this stage."""
        if v is None:
            return None
        field_name = info.field_name or "optional_field"
        # Use allow_empty=True here for mode='before'
        return validate_str_field(
            v,
            field_name=field_name,
            max_length=128,
            allow_empty=True,  # Allow empty
        )

    @field_validator("client_id", mode="after")
    @classmethod
    def check_client_id_not_empty_str(cls, v: str | None) -> str | None:
        """Ensure that if clientId is provided, it is not empty or just whitespace."""
        if v is not None and not v.strip():  # Check stripped value
