...
class BackpackAPI(ExchangeAPI):
    ...
    async def get_ticker(self, symbol: str) -> Ticker:
        ...
        try:
            ...
            # Defensive: Explicitly cast each field to expected type for static analysis
            symbol_val = cast(str, response["symbol"])
            bid_val = cast(str | float | int, response["bidPrice"])
            ask_val = cast(str | float | int, response["askPrice"])
            price_val = cast(str | float | int, response["lastPrice"])
            volume_val = cast(str | float | int, response["volume"])
            time_val = cast(str | float | int, response["time"])
            ticker = Ticker(
    ...
    async def get_order_book(self, symbol: str, depth: int = 20) -> OrderBook:
        ...
        def safe_list_of_pairs(raw: object) -> list[tuple[str, str]]:
            ...
            for entry in raw:
                # Defensive: ensure entry is a tuple or list of length 2
    ...
    async def get_balances(self) -> dict[str, Balance]:
        ...
        try:
            ...
            for asset, data in response.items():
                asset: Any = asset
                data: Any = data
                if not isinstance(asset, str) or not isinstance(data, dict):
                    logger.warning(
                        f"Unexpected asset/data types in Backpack balance response: {asset=}, {data=}, skipping."
                    )
                    continue
                # asset and data are now safe to use as str and dict[str, Any]
                try:
    ...
    def parse_order(self, data: dict[str, Any]) -> Order:
        ...
        try:
            ...
            filled_quantity = data.get("filledQuantity") if isinstance(data, dict) else None
            avg_fill_price = data.get("avgFillPrice") if isinstance(data, dict) else None
            # Backpack uses integer ms timestamps
            order_time_ms = data.get("time") if isinstance(data, dict) else None
            return Order(
                symbol=data["symbol"] if isinstance(data, dict) and "symbol" in data else "",
                side=OrderSide(data["side"])
                if isinstance(data, dict) and "side" in data
                else OrderSide.BUY,
                order_type=OrderType(data["orderType"])
                if isinstance(data, dict) and "orderType" in data
                else OrderType.LIMIT,
                quantity_requested=Decimal(str(quantity)) if quantity is not None else Decimal("0"),
                status=order_status,
                client_order_id=str(data.get("clientId", "")) if isinstance(data, dict) else "",
