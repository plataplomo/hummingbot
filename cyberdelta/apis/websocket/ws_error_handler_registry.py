...
_global_registry: WebSocketErrorHandlerRegistry | None = None


def get_global_registry() -> WebSocketErrorHandlerRegistry:
    """Get the global error handler registry instance.

    Returns:
        Global registry instance (created if doesn't exist)
    """
    global _global_registry

    if _global_registry is None:
        _global_registry = WebSocketErrorHandlerRegistry()

    return _global_registry


def set_global_registry(registry: WebSocketErrorHandlerRegistry) -> None:
    """Set the global error handler registry instance.
...
def clear_global_registry() -> None:
    """Clear the global registry instance.

    This forces creation of a new registry on next access.
    """
    global _global_registry
    if _global_registry is not None:
        _global_registry.clear_handlers()
...
def get_error_handler(
    ...
    """Convenience function to get an error handler from global registry.

    Args:
        exchange: Exchange name
        config: Optional error configuration
        environment: Environment name
        connection_manager: Optional connection manager
        subscription_manager: Optional subscription manager
        state_manager: Optional state manager
    ...
    Returns:
        Error handler for the exchange
    """
...
def remove_error_handler(exchange: ExchangeName, environment: str = "production") -> bool:
    """Convenience function to remove an error handler from global registry.
    ...
    """
    registry = get_global_registry()
    return registry.remove_handler(exchange=exchange, environment=environment)
