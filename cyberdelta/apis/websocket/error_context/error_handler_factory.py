...
class WebSocketErrorHandlerFactory:
    ...
    def create_handler_from_app_config(
        ...
        if callable(app_config_getter):
            # Callable that returns websocket config directly
            websocket_config = app_config_getter(exchange)
        else:
            # AppConfigProtocol instance with websocket_error property
            websocket_config = app_config_getter.websocket_error
