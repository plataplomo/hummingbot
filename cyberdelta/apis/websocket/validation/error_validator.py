...
class StreamErrorContextValidator(ErrorContextValidator):
    ...
    def validate_exchange(cls, exchange: str) -> str:
        ...
        """
        exchange = cls.validate_non_empty_string(exchange, "exchange").lower()

        if not cls.EXCHANGE_PATTERN.match(exchange):
            msg = (
                f"Invalid exchange format: {exchange!r}. "
                "Must be 2-32 lowercase characters with optional hyphens"
            )
            raise ValueError(msg)
        ...
        if exchange not in known_exchanges:
            # Just a warning via the error message, still allow unknown exchanges
            pass
