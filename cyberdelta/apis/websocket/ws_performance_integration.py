...
if __name__ == "__main__":
    ...
    messages = [backpack_msg, hyperliquid_msg]

    logger.info(
        "websocket_performance_integration_demo",
        component="PerformanceIntegration",
        action="demonstration",
    )

    # Demonstrate different performance modes
    for mode in [PerformanceMode.ULTRA_FAST, PerformanceMode.BALANCED, PerformanceMode.SECURE]:
        logger.info("performance_mode_test", mode=mode.upper())
        result = performance_processor.validate_message(backpack_msg, mode)
        logger.info("envelope_validated", envelope_type=result.envelope_type, performance_mode=mode)

    # Show performance stats
    logger.info("performance_statistics_header")
    stats = performance_processor.get_performance_stats()
    for key, value in stats.items():
        logger.info("performance_stat", stat_name=key, value=value)

    # Performance summary
    logger.info("performance_summary_header")
    summary = get_performance_summary()
    for mode, info in summary["modes"].items():
        logger.info("mode_performance_summary", mode=mode, performance=info["performance"])
