...
- Models in this file are used exclusively to validate and parse the *external* error response structures
  returned by Hyperliquid's REST and WebSocket APIs.
- All models enforce strict schema validation (`extra="forbid"`), strict type checking, and robust format validation (e.g., max length, valid UTF-8).
- Any unexpected, malformed, or ambiguous fields in upstream data are immediately rejected. This is critical for robust, secure, and predictable error handling in a financial system.
- These models are the *first step* in the "validate first, then transform" pattern: validate external data at the boundary, then map to internal error models or raise exceptions as appropriate.
