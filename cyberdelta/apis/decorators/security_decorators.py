        ...
        if field in data:
            ...
            if "min" in rules:
                ...
                if isinstance(min_val, int | float) and float(str(value)) < min_val:
                    raise ValueError(
                        f"Field {field} below minimum {min_val}: {value}"
                    )
            ...
            if "max" in rules:
                ...
                if isinstance(max_val, int | float) and float(str(value)) > max_val:
                    raise ValueError(
                        f"Field {field} exceeds maximum {max_val}: {value}"
                    )
...
def _check_negative_values(data: dict[str, object]) -> list[str]:
    ...
    anomalies: list[str] = []
    financial_fields = ["price", "quantity", "balance", "total", "available"]
    
    for field, value in data.items():
        if isinstance(value, str | int | float):
            try:
                if float(str(value)) < 0 and field in financial_fields:
...
def _validate_http_response(
    ...
    if not isinstance(raw_response_content, dict):
        raise APIError(
            message=(
                f"Expected dict for {raw_model.__name__}, "
    ...
    def decorator(
        ...
        async def wrapper(self: object, *args: object, **kwargs: object) -> BaseModel | None:
            ...
            if enable_security_monitoring:
                # Apply business logic validation to mapper function if constraints exist
                if business_constraints:
                    decorated_mapper = business_logic_validated(constraints=business_constraints)
