...
async def shutdown() -> None:
    ...
    if _trading_engine:
        try:
            ...
            logger.error("trading_engine_shutdown_error", error=str(e), exc_info=True)
    else:
...
def _load_configuration(args: argparse.Namespace) -> AppSettings:
    ...
    try:
        ...
        logger.error("configuration_error", error=str(e), config_path=args.config)
        sys.exit(1)
    except Exception as e:
        logger.error(
            "unexpected_configuration_error", error=str(e), config_path=args.config, exc_info=True
        )
        sys.exit(1)
...
async def main() -> None:
    ...
    try:
        ...
        logger.critical("critical_error_in_main", error=str(e), exc_info=True)
        await shutdown()
        sys.exit(1)
    finally:
