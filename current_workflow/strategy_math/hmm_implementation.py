...
class CointegrationAnalyzer:
    ...
    def test_pair_cointegration(self, price_data: pd.DataFrame) -> Tuple[bool, float, np.ndarray]:
        ...
        crit_value = result.cvt[0, int(90 + self.sig_level * 100) - 90]  # Critical value at sig_level
        ...
        p_value = 1.0 - (trace_stat / crit_value) if trace_stat < crit_value else self.sig_level / 2.0
    ...
    def find_cointegrated_pairs(self, price_data: pd.DataFrame) -> List[Tuple[str, str, np.ndarray]]:
        """
        ...
        for i in range(n):
            for j in range(i+1, n):
                ...
                if is_coint:
                    logger.info(f"Found cointegrated pair: {assets[i]} - {assets[j]} (p-value: {p_value:.4f})")
...
class HMMSpreadModel:
    ...
    def plot_spread_with_states(self, spread: pd.Series, figsize: Tuple[int, int] = (12, 6)):
        ...
        for i in range(self.n_states):
            ...
            plt.plot(spread.index[mask], spread.values[mask], '.', 
...
class StatisticalArbitrageStrategy:
    ...
    def initialize(self, price_data: pd.DataFrame) -> bool:
        ...
        logger.info(f"Assets {self.asset1} and {self.asset2} are cointegrated (p-value: {p_value:.4f})")
    ...
    def update(self, current_prices: pd.Series) -> Dict:
        ...
        if trade_action in ["EXIT_LONG", "EXIT_SHORT"]:
            if trade_action == "EXIT_LONG":
                ...
                pnl = (price1 - self.entry_price_1) - self.coint_vector[1] * (price2 - self.entry_price_2)
                pnl -= self.transaction_cost * (price1 + self.coint_vector[1] * price2)  # Transaction costs
                
            else:  # EXIT_SHORT
                # P&L from short spread (Sell asset1, buy asset2)
                pnl = (self.entry_price_1 - price1) - self.coint_vector[1] * (self.entry_price_2 - price2)
                pnl -= self.transaction_cost * (price1 + self.coint_vector[1] * price2)  # Transaction costs
...
class Backtest:
    ...
    def run(self):
        ...
        for i in range(training_period, len(self.price_data)):
            ...
            # Process trades
            if update_result["action"] in ["ENTER_LONG", "ENTER_SHORT", "EXIT_LONG", "EXIT_SHORT"]:
                # Record trade
