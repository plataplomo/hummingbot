        ...
        for _, row in current_data.iterrows():
            ...
            if symbol in self.positions:
                ...
                if (position['side'] == 'long' and funding_rate <= 0) or \
                   (position['side'] == 'short' and funding_rate >= 0) or \
                   abs(funding_rate) < self.min_funding_rate * 0.5:
                    ...
                    # Price P&L
                    price_pnl = (price - entry_price) / entry_price if position['side'] == 'long' else (entry_price - price) / entry_price
                    
                    # Funding P&L - simplified approximation
                    funding_rate_annualized = position['entry_funding_rate']
                    funding_pnl = (funding_rate_annualized / 8760) * time_held  # Convert annualized to hourly
            ...
            elif abs(funding_rate) >= self.min_funding_rate and len(self.positions) < self.max_positions:
