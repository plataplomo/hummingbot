...
class TestRiskManager:
    ...
    def test_size_opportunity_exceeds_constraints(self, risk_manager, sample_opportunity, portfolio_tracker):
        ...
        sized_opportunity = risk_manager.size_opportunity(sample_opportunity)
        assert isinstance(sized_opportunity, SizedOpportunity)
        assert sized_opportunity.position_size_usd <= 50.0 # Should be capped by the limit
    ...
    def test_validate_opportunities_all_invalid(self, risk_manager, sample_opportunity, portfolio_tracker):
        ...
        expired1 = ArbitrageOpportunity(
            symbol='ETH', timestamp=datetime.now() - timedelta(hours=2), 
            long_exchange='h', short_exchange='b', long_funding_rate=0, short_funding_rate=0,
            net_funding_differential=0, expected_profit=0, utility_score=0, basis_volatility=0
        )
        expired2 = ArbitrageOpportunity(
            symbol='SOL', timestamp=datetime.now() - timedelta(hours=3), 
            long_exchange='h', short_exchange='b', long_funding_rate=0, short_funding_rate=0,
            net_funding_differential=0, expected_profit=0, utility_score=0, basis_volatility=0
        )
