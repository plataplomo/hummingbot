...
from cyberdelta.core.types import Position, OrderType, SignalType, MarketData
from cyberdelta.utils.constants import Exchange
from cyberdelta.core.models import Ticker, FundingRate, Balance
...
class TestFundingRateArbitrageStrategy(unittest.TestCase):
    ...
    async def _async_test_check_opportunity(self, mock_logger):
        ...
        logger.info(f"Set up historical_basis data: {len(self.strategy.historical_basis['BTC-PERP'])} entries")
        ...
        self.strategy._estimate_slippage = MagicMock(return_value=0.0001)  # 0.01% slippage
        ...
        try:
            ...
            self.strategy._calculate_basis_volatility = MagicMock(return_value=0.005)  # Low volatility
