...
class TestPositionReconciliationSystem:
    ...
    def portfolio_tracker(self):
        ...
        tracker.get_api_client.side_effect = get_api_client
    ...
    async def test_check_positions(self, reconciliation_system):
        ...
        btc_discrepancy = next((d for d in hyper_results['discrepancies'] if d['symbol'] == 'BTC'), None)
    ...
    async def test_auto_correct(self, config, portfolio_tracker):
        ...
        btc_discrepancy = next(
            (d for d in system.discrepancy_history 
    ...
    def test_reconcile_positions(self, reconciliation_system):
        ...
        exchange_positions = [
            Position(symbol='BTC', size=1.0, entry_price=50000, mark_price=50000,
                     liquidation_price=45000, unrealized_pnl=0, leverage=1),
            Position(symbol='ETH', size=10.0, entry_price=3000, mark_price=3000,
                     liquidation_price=2700, unrealized_pnl=0, leverage=1),
        ]
        
        # Fill history positions with discrepancy
        fill_positions = [
            Position(symbol='BTC', size=0.9, entry_price=50000, mark_price=50000,  # 10% discrepancy
                     liquidation_price=45000, unrealized_pnl=0, leverage=1),
            Position(symbol='ETH', size=10.0, entry_price=3000, mark_price=3000,
                     liquidation_price=2700, unrealized_pnl=0, leverage=1),
            Position(symbol='SOL', size=50.0, entry_price=100, mark_price=100,  # Not in exchange
                     liquidation_price=90, unrealized_pnl=0, leverage=1),
        ]
        
        # Local positions with discrepancy
        local_positions = [
            Position(symbol='BTC', size=0.95, entry_price=50000, mark_price=50000,  # 5% discrepancy
                     liquidation_price=45000, unrealized_pnl=0, leverage=1),
            Position(symbol='ETH', size=10.0, entry_price=3000, mark_price=3000,
                     liquidation_price=2700, unrealized_pnl=0, leverage=1),
            Position(symbol='DOGE', size=1000.0, entry_price=0.1, mark_price=0.1,  # Not in exchange
                     liquidation_price=0.08, unrealized_pnl=0, leverage=1),
        ]
        ...
        sol_discrepancy = next((d for d in results['discrepancies'] if d['symbol'] == 'SOL'), None)
        assert sol_discrepancy is not None
        
        # Check DOGE (should be detected since in local but not exchange)
        doge_discrepancy = next((d for d in results['discrepancies'] if d['symbol'] == 'DOGE'), None)
    ...
    def test_get_reconciliation_report(self, reconciliation_system):
        ...
        assert report['reconciliation_threshold'] == reconciliation_system.reconciliation_threshold 