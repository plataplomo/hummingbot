...
class TestBackpackAccountSummaryZeroBalance:
    ...
    async def test_get_account_summary_empty_account(
        ...
        logger.info(f"✓ Account summary zero balance test completed: equity={account_summary.total_equity}")
    ...
    async def test_get_account_summary_precision_validation_zero_balance(
        ...
        for field_name, field_value in financial_fields:
            ...
            if field_value == Decimal("0"):
                assert str(field_value) == "0", f"Zero {field_name} should be exact"
                logger.info(f"✓ Zero {field_name} has exact precision")
         ...
         if account_summary.total_equity == Decimal("0"):
             assert account_summary.available_equity == Decimal("0"), "Zero equity should mean zero available"
    ...
    async def test_get_account_summary_exchange_specific_fields_zero_balance(
        ...
        margin_fields = ["total_equity", "available_balance", "used_balance", "withdrawable_balance"]
        ...
        for field in margin_fields:
            ...
            # Validate consistency with main object
            main_field_value = getattr(account_summary, field)
            assert field_value == main_field_value, (
                f"Backpack {field} ({field_value}) != main {field} ({main_field_value})"
            )
            
            logger.info(f"✓ Backpack {field} = {field_value} (consistent)")
        
        # Test zero balance specific validations
        if account_summary.total_equity == Decimal("0"):
            assert bp_details.total_equity == Decimal("0")
            assert bp_details.available_balance == Decimal("0") 
            assert bp_details.used_balance == Decimal("0")
            # withdrawable_balance might be zero or negative (fees)
    ...
    async def test_get_account_summary_error_handling_zero_balance(
        ...
        try:
            ...
            contains_relevant_keyword = any(
                keyword in error_msg for keyword in account_keywords + auth_keywords
            )
    ...
    async def test_get_account_summary_consistency_zero_balance(
        ...
        if len(successful_summaries) >= 2:
            ...
            for i, subsequent_summary in enumerate(successful_summaries[1:], 1):
                ...
                assert first_summary.available_balance == subsequent_summary.available_balance  
                assert first_summary.used_balance == subsequent_summary.used_balance
                assert first_summary.withdrawable_balance == subsequent_summary.withdrawable_balance
    ...
    async def test_get_account_summary_field_bounds_zero_balance(
        ...
        min_value = Decimal("-1000")  # Allow for small negative values (fees)
        max_value = Decimal("1000")   # Zero balance shouldn't have large values
        
        field_bounds = [
            ("total_equity", Decimal("0"), max_value),           # Should be >= 0
            ("available_balance", Decimal("0"), max_value),      # Should be >= 0  
            ("used_balance", Decimal("0"), max_value),           # Should be >= 0
            ("withdrawable_balance", min_value, max_value),      # Can be negative (fees)
        ]
        
        for field_name, min_bound, max_bound in field_bounds:
            field_value = getattr(account_summary, field_name)
            ...
            assert decimal_places <= 18, f"{field_name} has excessive precision: {decimal_places}"
            
            logger.info(f"✓ {field_name} = {field_value} within bounds [{min_bound}, {max_bound}]")
        
        # Test logical relationships
        if account_summary.total_equity == Decimal("0"):
            # For true zero balance accounts
            assert account_summary.available_balance == Decimal("0")
            assert account_summary.used_balance == Decimal("0")
            logger.info("✓ True zero balance account relationships validated")
        else:
            # For accounts with minimal equity
            assert account_summary.total_equity > Decimal("0")
            total_balance = account_summary.available_balance + account_summary.used_balance
            assert total_balance <= account_summary.total_equity + Decimal("0.01")  # Allow small rounding
