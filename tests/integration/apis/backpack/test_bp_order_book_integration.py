...
@pytest.mark.parametrize("custom_vcr_cassette_dir", ["apis/backpack/order_book"], indirect=True)
@pytest.mark.asyncio
...
async def test_bp_get_order_book_btc_usdc_success(
    ...
    # Validate core order book fields
    assert order_book.symbol == "BTC_USDC", f"Expected symbol 'BTC_USDC', got '{order_book.symbol}'"

    # Basic structure validation
    assert isinstance(order_book.bids, list), f"Bids should be list, got {type(order_book.bids)}"
    assert isinstance(order_book.asks, list), f"Asks should be list, got {type(order_book.asks)}"
    assert len(order_book.bids) > 0, "Should have at least one bid for liquid BTC_USDC market"
    assert len(order_book.asks) > 0, "Should have at least one ask for liquid BTC_USDC market"
...
async def test_bp_get_order_book_multiple_symbols_consistency(
    ...
    for order_book in order_books:
        ...
        assert isinstance(order_book.bids, list), f"Bids should be list for {order_book.symbol}"
        assert isinstance(order_book.asks, list), f"Asks should be list for {order_book.symbol}"
