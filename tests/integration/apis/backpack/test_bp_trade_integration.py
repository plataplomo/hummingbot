...
@pytest.mark.parametrize("custom_vcr_cassette_dir", ["apis/backpack/trade"], indirect=True)
@pytest.mark.asyncio
...
async def test_bp_get_recent_trades_limit_parameter(
    ...
    # Validate return type
    assert isinstance(trades_small, list), f"Expected list of trades, got {type(trades_small)}"
    ...
    # Validate return type
    assert isinstance(trades_large, list), f"Expected list of trades, got {type(trades_large)}"
    ...
    if len(trades_small) == small_limit and len(trades_large) > small_limit:
        ...
        for i in range(min(len(trades_small), len(trades_large))):
            ...
            if hasattr(small_trade, "executed_at") and hasattr(large_trade, "executed_at"):
...
async def test_bp_get_recent_trades_precision_validation(
    ...
    if len(trades) > 0:
        for i, trade in enumerate(trades):
            ...
            assert isinstance(total_value, Decimal), f"Trade {i} total value should be Decimal"
...
async def test_bp_get_recent_trades_zero_limit_handling(
    ...
    try:
        ...
        assert isinstance(trades, list), f"Expected list of trades, got {type(trades)}"
        assert len(trades) == 0, f"Expected empty list for limit=0, got {len(trades)} trades"
...
async def test_bp_get_recent_trades_multiple_symbols_consistency(
    ...
    for symbol in symbols:
        ...
        assert isinstance(trades, list), f"Expected list for {symbol}, got {type(trades)}"
