"""Integration tests for Backpack FundingRate model pipeline.
...
@pytest.mark.parametrize(
    "custom_vcr_cassette_dir", ["apis/backpack/funding_rate"], indirect=True
)
@pytest.mark.asyncio
...
async def test_bp_get_funding_rate_sol_perp_success(
    ...
    assert isinstance(funding_rate, FundingRate), (
        ...
        f"Expected FundingRate, got {type(funding_rate)}"
...
async def test_bp_get_funding_rates_chronological_ordering(
    ...
    if len(funding_rates) > 1:
        for i in range(len(funding_rates) - 1):
            ...
            if (hasattr(current_rate, "timestamp") and
                hasattr(next_rate, "timestamp")):
...
async def test_bp_get_funding_rate_spot_symbol_error(
    ...
    assert (
        "SOL_USDC" in str(error) or
        "funding" in str(error).lower() or
        "perpetual" in str(error).lower()
    )
...
async def test_bp_get_funding_rate_nonexistent_symbol_error(
    ...
    # Validate error details
    error = exc_info.value
    assert ("NOTREAL_USDC_PERP" in str(error) or "not found" in str(error).lower() or 
