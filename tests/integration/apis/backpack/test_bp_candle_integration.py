...
@pytest.mark.parametrize(
    "custom_vcr_cassette_dir", ["apis/backpack/candle"], indirect=True
)
@pytest.mark.asyncio
    ...
    if len(candles) > 0:
        for i, candle in enumerate(candles):
            ...
            assert candle.open > Decimal("1000"), (
                f"BTC open price seems too low: {candle.open}"
            )
            assert candle.high > Decimal("1000"), (
        f"BTC high price seems too low: {candle.high}"
    )
            assert candle.low > Decimal("1000"), (
        f"BTC low price seems too low: {candle.low}"
    )
...
async def test_bp_get_market_data_chronological_ordering(
    ...
    if len(candles) > 1:
        for i in range(len(candles) - 1):
            ...
            if (hasattr(current_candle, "open_time") and
...
async def test_bp_get_market_data_future_time_range_handling(
    ...
    try:
        ...
        assert isinstance(candles, list), (
            ...
            f"Expected list, got {type(candles)}"
...
async def test_bp_get_market_data_multiple_symbols_consistency(
    ...
    for symbol in symbols:
        ...
        assert isinstance(candles, list), (
            ...
            f"Expected list for {symbol}, got {type(candles)}"
