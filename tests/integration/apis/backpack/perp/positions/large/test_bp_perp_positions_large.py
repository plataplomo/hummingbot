...
class TestBackpackPerpLargePositions:
    ...
    async def _get_exchange_max_order_quantity(
        ...
        return await api.account_service._get_exchange_max_order_quantity(max_order_args) # type: ignore[protected-access]
            ...
            if len(max_sizes) == 2:
                ...
                for symbol, data in max_sizes.items():
                    ...
                    assert notional > Decimal("0"), (
                        f"{symbol} should have positive max notional"
                    )
