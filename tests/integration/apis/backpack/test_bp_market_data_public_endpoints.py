"""Integration tests for Backpack public endpoints using real API client.
...
@pytest.mark.vcr
async def test_bp_get_ticker_integration(
    bp_api_for_test_env: BackpackAPI,
    custom_vcr_config: dict[str, Any],
) -> None:
    """Test BackpackAPI.get_ticker() method returns valid Ticker internal model.
    ...
    # Validate price precision (should be reasonable for SOL)
...
@pytest.mark.integration
@pytest.mark.vcr
async def test_bp_get_ticker_integration_btc(
    bp_api_for_test_env: BackpackAPI,
    custom_vcr_config: dict[str, Any],
) -> None:
    """Test BackpackAPI.get_ticker() method with BTC_USDC symbol."""
...
@pytest.mark.integration
@pytest.mark.vcr
async def test_bp_get_order_book_integration(
    bp_api_for_test_env: BackpackAPI,
    custom_vcr_config: dict[str, Any],
) -> None:
    """Test BackpackAPI.get_order_book() method returns valid OrderBook internal model.
    ...
    assert order_book.symbol == "SOL_USDC", f"Expected symbol 'SOL_USDC', got '{order_book.symbol}'"
...
@pytest.mark.integration
@pytest.mark.vcr
async def test_bp_get_recent_trades_integration(
    bp_api_for_test_env: BackpackAPI,
    custom_vcr_config: dict[str, Any],
) -> None:
    """Test BackpackAPI.get_recent_trades() method returns valid Trade internal models.
    ...
    if len(trades) > 0:
        ...
        for i, trade in enumerate(trades):
            ...
            # Validate symbol
            assert trade.symbol == "SOL_USDC", f"Trade {i} symbol should be 'SOL_USDC', got '{trade.symbol}'"
...
@pytest.mark.integration
@pytest.mark.vcr
async def test_bp_get_funding_rate_integration(
    bp_api_for_test_env: BackpackAPI,
    custom_vcr_config: dict[str, Any],
) -> None:
    """Test BackpackAPI.get_funding_rate() method returns valid FundingRate internal model.
    ...
    assert hasattr(funding_rate, "rate"), "FundingRate should have rate attribute"
    assert hasattr(funding_rate, "timestamp"), "FundingRate should have timestamp attribute"
    
    # Validate data types
    assert isinstance(funding_rate.rate, Decimal), f"Rate should be Decimal, got {type(funding_rate.rate)}"
    ...
    assert abs(funding_rate.rate) < Decimal("1"), (
        f"Funding rate seems unreasonable: {funding_rate.rate}"
    )
...
@pytest.mark.integration
@pytest.mark.vcr
async def test_bp_get_funding_rates_integration(
    bp_api_for_test_env: BackpackAPI,
    custom_vcr_config: dict[str, Any],
) -> None:
    """Test BackpackAPI.get_funding_rates() method returns valid FundingRate internal models.
...
@pytest.mark.integration
@pytest.mark.vcr
async def test_bp_get_market_data_integration(
    bp_api_for_test_env: BackpackAPI,
    custom_vcr_config: dict[str, Any],
) -> None:
    """Test BackpackAPI.get_market_data() method returns valid Candle internal models.
    ...
    if len(candles) > 0:
        for i, candle in enumerate(candles):
            ...
            # Validate symbol
            assert candle.symbol == "SOL_USDC", f"Candle {i} symbol should be 'SOL_USDC', got '{candle.symbol}'"