...
integrates with rate limiting, including both HTTP API rate limiting and
...
class TestHyperliquidRateLimitingIntegration:
    ...
    async def test_websocket_rate_limiter_initialization(
        ...
        ws_manager = getattr(hl_api_for_test_env, "_ws_manager", None)
        if not ws_manager:
    ...
    async def test_http_api_rate_limiting(
        ...
        rate_limit_strategy = getattr(hl_api_for_test_env, "_rate_limit_strategy", None)
        ...
        for i in range(request_count):
            ...
            try:
                ...
                await hl_api_for_test_env.get_markets(GetMarketsArgs())
    ...
    async def test_rate_limiter_token_consumption(
        ...
        try:
            ...
            if hl_api_for_test_env.is_connected:
                # Test token consumption
                markets = await hl_api_for_test_env.get_markets(GetMarketsArgs())
                if markets:
                    ...
                    await hl_api_for_test_env.subscribe(
                        f"l2Book:{markets[0].symbol}", token_test_handler
                    )
        ...
        try:
            ...
            for i in range(rapid_requests):
                try:
                    ...
                    await hl_api_for_test_env.subscribe(f"l2Book:{symbol}", rate_limit_test_handler)
    ...
    @pytest.mark.asyncio
    async def test_hyperliquid_specific_rate_limits(
        self,
        hl_api_for_test_env: HyperliquidAPI,
    ) -> None:
        """Test Hyperliquid-specific rate limiting behavior."""
        try:
            await hl_api_for_test_env.connect_websocket()
            ...
            # Test Hyperliquid-specific endpoints that may have different rate limits
            specific_subscriptions = [
                "allMids",  # Global market data
                "userEvents",  # User-specific events
                "userFills",  # User fills
                "userFundings",  # User funding
            ]
            ...
            for topic in specific_subscriptions:
                try:
                    await hl_api_for_test_env.subscribe(topic, specific_handler)
                    subscription_results[topic] = "success"
                    logger.info(
                        "hyperliquid_specific_subscription_success",
                        topic=topic,
                        message=f"✓ Successfully subscribed to {topic}",
                    )
                except (ValidationError, ValueError, TypeError, KeyError, AttributeError) as e:
                    subscription_results[topic] = f"failed: {str(e)[:50]}"
                    logger.info(
                        "hyperliquid_specific_subscription_failed",
                        topic=topic,
                        error=str(e)[:100],
                        message=f"Subscription to {topic} failed (may require auth)",
                    )

            logger.info(
                "hyperliquid_specific_rate_limits_tested",
                results=subscription_results,
                successful_count=len([r for r in subscription_results.values() if r == "success"]),
                message="✓ Hyperliquid-specific rate limits tested",
            )

        except (ValidationError, ValueError, TypeError, KeyError, AttributeError) as e:
            pytest.fail(f"Hyperliquid-specific rate limit test failed: {e}")
