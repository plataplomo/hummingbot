...
async def test_hl_get_perp_order_book_btc_success(
    ...
    order_book = await hl_api_for_test_env.get_order_book(exchanges.hyperliquid("BTC"))
    ...
    assert order_book.symbol.value == "BTC", (
        f"Expected symbol 'BTC', got '{order_book.symbol.value}'"
    )
...
async def test_hl_get_perp_order_book_nonexistent_symbol_raises_api_error(
    ...
    with pytest.raises(APIError) as exc_info:
        await hl_api_for_test_env.get_order_book(exchanges.hyperliquid("NONEXISTENT"))

    assert exc_info.value.code == APIErrorCode.INVALID_RESPONSE.value
    assert "Expected dict response for l2Book, got <class 'str'>" in exc_info.value.message
