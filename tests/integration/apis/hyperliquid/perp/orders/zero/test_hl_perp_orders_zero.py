"""Integration tests for Hyperliquid perpetual order info endpoints.

This module focuses specifically on testing the Order model pipeline
through Hyperliquid's /info (unsigned) endpoints for perpetual contracts.
Tests validate complete data transformation from API responses to Order instances.

Model Focus: Order (Read Operations - Perpetual)
- Validates complete Order model field mapping for perpetual orders
- Tests Decimal precision for financial values (quantities, prices)
- Validates business logic constraints for perpetual order retrieval
- Tests Hyperliquid-specific order details (hl_details with order management info)
- Comprehensive error handling for perpetual order queries
...
class TestHyperliquidPerpOrdersZero:
    ...
    This class tests /info (read) operations only for perpetual contracts:

    /info endpoint (unsigned, user address in body):
    - get_order_by_id (perpetual orders)
    - get_order_history (perpetual orders)
    - get_open_orders (perpetual orders)
    ...
    async def test_get_order_by_id_success_comprehensive(
        ...
        This tests perpetual order retrieval by ID and validates the Order model consistency.
    ...
    async def test_get_order_history_success_comprehensive(
        ...
        This tests historical perpetual order retrieval and validates Order model consistency.
        """
