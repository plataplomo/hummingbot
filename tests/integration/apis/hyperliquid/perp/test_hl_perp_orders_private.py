...
class TestHyperliquidPerpOrdersPrivate:
    ...
    async def test_place_order_invalid_symbol(
        ...
        valid_symbol = await get_test_symbol(
            hl_api_for_test_env, "perp", 0
        )  # Already returns Symbol
    ...
    async def test_cancel_all_orders_no_open_orders(
        ...
        try:
            ...
            # Wait for all orders to be actually cancelled
            await HyperliquidTestHelpers.wait_for_order_cancellation(hl_api_for_test_env)
        except (APIError, ValueError, TypeError, KeyError) as cleanup_error:
            logger.debug(
                "cleanup_cancellation_failed",
                error=str(cleanup_error),
