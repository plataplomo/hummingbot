...
@pytest.mark.parametrize(
    "custom_vcr_cassette_dir", ["apis/hyperliquid/ticker"], indirect=True
)
@pytest.mark.asyncio
...
async def test_hl_get_ticker_btc_success(
    ...
    assert isinstance(ticker, Ticker), (
        ...
        f"Expected Ticker, got {type(ticker)}"
    ...
    assert ticker.symbol == "BTC", (
        ...
        f"Expected symbol 'BTC', got '{ticker.symbol}'"
...
async def test_hl_get_ticker_eth_success(
    ...
    assert ticker.symbol == "ETH", (
        ...
        f"Expected symbol 'ETH', got '{ticker.symbol}'"
...
async def test_hl_get_ticker_sol_success(
    ...
    assert ticker.symbol == "SOL", (
        ...
        f"Expected symbol 'SOL', got '{ticker.symbol}'"
...
async def test_hl_get_ticker_avax_success(
    ...
    try:
        ...
        if ticker is not None:
            ...
            # Validate core ticker fields
            assert ticker.symbol == "AVAX", (

                f"Expected symbol 'AVAX', got '{ticker.symbol}'"
    ...
    for symbol in symbols:
        ...
        if ticker is not None:
            assert isinstance(ticker, Ticker), (
                ...
                f"Expected Ticker for {symbol}, got {type(ticker)}"
...
async def test_hl_get_ticker_performance_consistency(
    ...
    assert len(symbols) == 1, (
        ...
        f"All calls should return same symbol, got: {symbols}"
    ...
    assert len(unique_prices) == 1, (
        ...
        f"VCR recorded prices should be identical, got: {unique_prices}"
