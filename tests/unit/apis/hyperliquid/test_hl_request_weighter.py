...
class TestHyperliquidRequestWeighterEdgeCases:
    ...
    def test_none_default_weights_uses_fallback(self) -> None:
        ...
        weighter = HyperliquidRequestWeighter(config)
        
        # Should use fallback values
        ip_weight_info = weighter.get_ip_weight("/info", {"type": "unknown"})
        ip_weight_exchange = weighter.get_ip_weight("/exchange", {"actions": [{}]})
        
        assert ip_weight_info == 20  # Fallback in code
        assert ip_weight_exchange == 1  # Fallback in code (0 + 0 // 40 = 0, but uses 1 as fallback)
