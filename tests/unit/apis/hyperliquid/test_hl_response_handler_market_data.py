...
class TestHandleInfoMetaAndAssetCtxsResponse:
    ...
    def test_validation_error_wrong_structure(self) -> None:
        ...
        assert "2-element list" in exc_info.value.message
    ...
    def test_validation_error_missing_universe(self) -> None:
        ...
        assert exc_info.value.code == APIErrorCode.INVALID_RESPONSE.value
        assert "Invalid info_meta_and_asset_ctxs response from exchange:" in exc_info.value.message
...
class TestHandleInfoFundingRateResponse:
    ...
    def test_valid(self, valid_raw_asset_ctx: dict[str, Any], symbol: str) -> None:
        ...
        assert response.name == ETH_HL.value
    ...
    def test_validation_error_missing_funding(self, symbol: str) -> None:
        ...
        raw_data = {"name": ETH_HL.value, "markPx": "3000.0"}  # Missing funding
        with pytest.raises(APIError) as exc_info:
        ...
        assert exc_info.value.code == APIErrorCode.INVALID_RESPONSE.value
        assert "Invalid info_funding_rate response from exchange:" in exc_info.value.message
        assert isinstance(exc_info.value.original_exception, (ValidationError, TypeError))
    ...
    def test_invalid_top_level_type(self, symbol: str) -> None:
        ...
        assert exc_info.value.code == APIErrorCode.INVALID_RESPONSE.value
        assert "Unexpected info_funding_rate response format:" in exc_info.value.message
...
class TestHandleInfoL2BookResponse:
    ...
    def test_valid(self, valid_raw_l2_book: dict[str, Any], symbol: str) -> None:
        ...
        assert response.coin == ETH_HL.value
    ...
    def test_validation_error_missing_levels(self, symbol: str) -> None:
        ...
        raw_data = {"coin": ETH_HL.value, "time": 1678889300000}  # Missing levels
        with pytest.raises(APIError) as exc_info:
        ...
        assert exc_info.value.code == APIErrorCode.INVALID_RESPONSE.value
        assert "Invalid info_l2_book" in exc_info.value.message
        assert "response from exchange:" in exc_info.value.message
        assert isinstance(exc_info.value.original_exception, (ValidationError, TypeError))
...
class TestHandleInfoRecentTradesResponse:
    ...
    def test_valid(self, valid_raw_public_trade: dict[str, Any], symbol: str) -> None:
        ...
        assert response_list[0].coin == ETH_HL.value
    ...
    def test_validation_error_invalid_trade_item(self, symbol: str) -> None:
        ...
        invalid_trade = {"coin": ETH_HL.value}  # Missing required fields
...
class TestHandleInfoCandleSnapshotResponse:
    ...
    def test_candle_snapshot_mismatched_arrays(self, symbol: str) -> None:
        """Test candle snapshot with mismatched array lengths causes ParsingError."""
        raw_data = {
