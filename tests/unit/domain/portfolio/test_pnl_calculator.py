...
from cyberdelta.config.models import AppSettings, CalculationConfig
...
from cyberdelta.models.portfolio.portfolio_state import PortfolioState
...
def position_strategy() -> SearchStrategy[dict]:
...
def mock_config():
    ...
    config.calculation = MagicMock(spec=CalculationConfig)
...
@pytest.fixture
def pnl_calculator(mock_config, mock_state_manager):
...
class TestPnLCalculationProperties:
    ...
    def test_return_percentage_calculation(
        ...
        if entry_value > 0:
            expected_return = (pnl / entry_value) * 100
        else:
    ...
    def test_portfolio_pnl_aggregation(self, positions: list[dict]):
        ...
        for pos in positions:
            ...
            if pos["side"] == OrderSide.BUY:
                position_pnl = (pos["current_price"] - pos["entry_price"]) * abs(pos["size"])
            else:
                position_pnl = (pos["entry_price"] - pos["current_price"]) * abs(pos["size"])

            total_unrealized_pnl += position_pnl

        # Property: Total PnL should be sum of individual PnLs
        calculated_total = sum(
            (pos["current_price"] - pos["entry_price"]) * abs(pos["size"])
            if pos["side"] == OrderSide.BUY
            else (pos["entry_price"] - pos["current_price"]) * abs(pos["size"])
            for pos in positions
        )
...
class TestPnLCalculatorIntegration:
    ...
    async def test_position_pnl_calculation(
        ...
        symbol = Symbol(value="BTC", exchange_configs={})
        ...
        state = MagicMock(spec=PortfolioState)
        state.positions = {f"{exchange.value}:{symbol.value}": position}
        mock_state_manager.get_state.return_value = state

        # Mock market price
        pnl_calculator._get_current_market_price = AsyncMock(return_value=current_price)

        # Calculate PnL
        result = await pnl_calculator.calculate_position_pnl(symbol, exchange)
    ...
    async def test_comprehensive_pnl_report(
        self, pnl_calculator, mock_state_manager, include_fees: bool, total_equity: Decimal
    ):
    ...
    @pytest.mark.asyncio
    async def test_missing_portfolio_equity_error(self, pnl_calculator, mock_state_manager):
...
class TestPnLEdgeCases:
    ...
    def test_zero_quantity_pnl(self, entry_price: Decimal, current_price: Decimal):
    ...
    @given(
        ...
        large_quantity=st.decimals(
            min_value=Decimal("10000"), max_value=Decimal("1000000"), places=2
        ),
    )
    def test_precision_preservation(self, very_small_price: Decimal, large_quantity: Decimal):
...
class TestFeeHandlingProperties:
    ...
    def test_fee_impact_on_pnl(self, gross_pnl: Decimal, fee_amount: Decimal):
    ...
    def test_total_fees_aggregation(self, fees: list[Decimal]):
...
class TestMathematicalInvariants:
    ...
    def test_pnl_additivity(self, positions: list[dict]):
        ...
        for pos in positions:
            if pos["side"] == OrderSide.BUY:
                pnl = (pos["current_price"] - pos["entry_price"]) * abs(pos["size"])
            else:
                pnl = (pos["entry_price"] - pos["current_price"]) * abs(pos["size"])
    ...
    @given(
        ...
        percentage_change=st.decimals(
            min_value=Decimal("-0.99"), max_value=Decimal("10"), places=4
        ),
        quantity=quantity_strategy(),
    )
