...
class TestHyperliquidAPI:
    ...
    async def test_get_balances(self, api_client, mock_session):
        ...
        api_client._request = AsyncMock(
            return_value={"data": {"walletBalanceUsd": "1000.50"}}
        )
    ...
    async def test_cancel_order(self, api_client, mock_session):
        ...
        api_client._request = AsyncMock(
            return_value={"status": "ok", "data": {"statuses": ["CANCELED"]}}
        )
    ...
    async def test_authentication(self, api_client):
        ...
        auth_data = await api_client._authenticate(
            "POST", "/exchange/test", data={"type": "order", "test": True}
        )
