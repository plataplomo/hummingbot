...
class TestRiskManager:
    ...
    def mock_portfolio_tracker(self):
        ...
        tracker.get_exchange_balance.return_value = 50000.0 # Default if not using side_effect
        tracker.get_total_exposure.return_value = 10000.0
        tracker.get_symbol_exposure.return_value = 0.0 # Default value
        tracker.get_exchange_exposure.return_value = 0.0 # RE-CONFIRM: Ensure this returns 0.0
    ...
    def test_check_portfolio_constraints(self, risk_manager, mock_portfolio_tracker):
        ...
        mock_portfolio_tracker.get_exchange_balance.side_effect = lambda exchange: 50000.0 
        mock_portfolio_tracker.get_exchange_exposure.side_effect = lambda exchange: 0.0
        assert risk_manager._check_portfolio_constraints(
            'hyperliquid', 'backpack', 1000.0, 1000.0) is True
        ...
        assert risk_manager._check_portfolio_constraints(
            'hyperliquid', 'backpack', 1000.0, 1000.0) is False
        ...
        mock_portfolio_tracker.get_exchange_balance.side_effect = lambda ex: 100.0 if ex == 'hyperliquid' else 50000.0
        mock_portfolio_tracker.get_exchange_exposure.side_effect = lambda exchange: 0.0 
        assert risk_manager._check_portfolio_constraints(
            'hyperliquid', 'backpack', 1000.0, 1000.0) is False
        ...
        mock_portfolio_tracker.get_exchange_balance.return_value = 50000.0 # Reset to default from fixture
        mock_portfolio_tracker.get_exchange_exposure.return_value = 0.0 # Reset to default from fixture
    ...
    def test_validate_opportunities(self, risk_manager, sample_opportunity):
        ...
        expired_opportunity = ArbitrageOpportunity(
            symbol='ETH', long_exchange='h', short_exchange='b', 
            long_funding_rate=0, short_funding_rate=0, net_funding_differential=0, 
            timestamp=datetime.now() - timedelta(hours=2), # Expired
            expected_profit=0, utility_score=0, basis_volatility=0
        )
