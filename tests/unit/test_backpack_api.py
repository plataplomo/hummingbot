...
class TestBackpackAPI:
    ...
    def mock_session(self, monkeypatch, mock_client_session):
        ...
        responses = {
            ...
            (
                "GET",
                "https://api.backpack.exchange/api/v1/ticker/BTCUSDC",
            ): MockResponse(
                {
                    "symbol": "BTCUSDC",
                    "bidPrice": "42450.50",
    ...
    async def test_get_ticker(self, api_client, mock_session):
        ...
        api_client._request.assert_called_once_with("GET", "/api/v1/ticker/BTCUSDC")
    ...
    async def test_get_order_book(self, api_client, mock_session):
        ...
        api_client._request = AsyncMock(
            return_value={
                "lastUpdateId": 123456789,
                "bids": [["42450.50", "0.5"], ["42440.25", "1.2"], ["42430.00", "2.5"]],
                "asks": [["42550.75", "0.3"], ["42560.50", "0.8"], ["42570.25", "1.5"]],
                "time": str(mock_time),
            }
        )
    ...
    async def test_get_recent_trades(self, api_client, mock_session):
        ...
        assert trades[0].timestamp == mock_time1

        # Verify second trade
        assert trades[1].symbol == "BTCUSDC"
        assert trades[1].id == "12346"
        assert trades[1].price == Decimal("42505.50")
        assert trades[1].quantity == Decimal("0.03")
        assert trades[1].side == OrderSide.SELL
        assert trades[1].timestamp == mock_time2
    ...
    async def test_cancel_order(self, api_client, mock_session):
        ...
        api_client._request.assert_called_once_with(
            "DELETE",
            "/api/v1/order",
            params={"symbol": "BTCUSDC", "orderId": "123456789"},
            signed=True,
        )
