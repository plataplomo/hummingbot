...
def test_discrepancy_detail_happy_path() -> None:
    ...
    assert detail.symbol == BTC_HL.value
...
def test_discrepancy_detail_minimal_required_fields() -> None:
    ...
    assert detail.symbol == ETH_HL.value
...
def test_discrepancy_detail_optional_fields_provided() -> None:
    ...
    assert detail.symbol == SOL_HL.value
...
def test_discrepancy_detail_frozen() -> None:
    ...
    with pytest.raises(ValidationError) as exc_info:
        ...
        detail.symbol = ETH_HL.value  # Use setattr to test runtime frozen validation
...
def test_historical_record_happy_path() -> None:
    ...
    discrepancy = DiscrepancyDetail(symbol=BTC_HL.value, discrepancy_type="size")
...
def test_historical_record_default_is_corrected() -> None:
    ...
    discrepancy = DiscrepancyDetail(symbol=ETH_HL.value, discrepancy_type="entry_price")
    now = datetime.now(UTC)
    data_to_validate = {
        "detail": discrepancy,
...
def test_historical_record_missing_required_field(missing_field: str) -> None:
    """Test ValidationError when a required field is missing."""
    discrepancy = DiscrepancyDetail(symbol=SOL_HL.value, discrepancy_type="mark_price")
    now = datetime.now(UTC)
    data_dict = {
        "detail": discrepancy,
