...
class TestExecutionHandler:
    ...
    @pytest.mark.asyncio
    async def test_circuit_breaker_open_rejection(self, execution_handler, sized_opportunity, mock_circuit_breaker_system):
        ...
        mock_circuit_breaker_system.can_execute.side_effect = lambda ex, sym: (False, breaker_message) if ex == "hyperliquid" else (True, None)
    ...
    @pytest.mark.asyncio
    async def test_execute_opportunity_mapping_failure(self, execution_handler, sized_opportunity, mock_symbol_mapper):
    ...
    @pytest.mark.asyncio
    async def test_place_order_with_retry_success(self, execution_handler, mock_hl_api, mock_portfolio_tracker):
        """Test successful order placement via retry helper."""
        mock_order = Order(
            id="order1", symbol="BTC-PERP", side=OrderSide.BUY, type=OrderType.MARKET,
            quantity=Decimal("1.0"), status=OrderStatus.FILLED
        )
        mock_hl_api.place_order.return_value = mock_order

        result_order = await execution_handler._place_order_with_retry(
            client=mock_hl_api, exchange_id="hyperliquid", symbol="BTC-PERP",
            side=OrderSide.BUY, order_type=OrderType.MARKET, quantity=Decimal("1.0")
        )

        assert result_order == mock_order
        mock_hl_api.place_order.assert_called_once_with(
            symbol="BTC-PERP", side=OrderSide.BUY, order_type=OrderType.MARKET,
            quantity=Decimal("1.0"), price=None, time_in_force="GTC", reduce_only=False
        )
        mock_portfolio_tracker.update_order.assert_called_once_with("hyperliquid", mock_order)

    @pytest.mark.asyncio
    async def test_place_order_with_retry_failure(self, execution_handler, mock_hl_api, mock_circuit_breaker_system):
    ...
    @pytest.mark.asyncio
    async def test_get_order_status_success(self, execution_handler, mock_hl_api, mock_portfolio_tracker):
        ...
        result = await execution_handler._get_order_status(mock_hl_api, "hyperliquid", "order1", "BTC-PERP")
    ...
    @pytest.mark.asyncio
    async def test_get_order_status_failure(self, execution_handler, mock_hl_api, mock_circuit_breaker_system):
        """Test failure when retrieving order status."""
        api_error = APIError("Not found", code=APIErrorCode.ORDER_NOT_FOUND, exchange_code="hyperliquid")
        ...
        mock_hl_api.get_order_status.assert_called_once_with(order_id="order1", symbol="BTC-PERP")
        mock_circuit_breaker_system.record_api_error.assert_called_once_with("hyperliquid", f"Get order status failed: {api_error}")
    ...
    async def test_compensate_position_success(self, execution_handler, mock_hl_api, mock_symbol_mapper):
        ...
        mock_comp_order = Order(id="comp1", symbol="BTC-PERP", side=OrderSide.SELL, type=OrderType.LIMIT, quantity=Decimal("0.5"), status=OrderStatus.FILLED)

        # Patch the internal _place_order_with_retry method
        with patch.object(execution_handler, '_place_order_with_retry', new_callable=AsyncMock) as mock_place_retry:
    ...
    async def test_compensate_position_mapping_failure(self, execution_handler, mock_hl_api, mock_symbol_mapper, mock_circuit_breaker_system):
        ...
        with patch.object(execution_handler, '_place_order_with_retry', new_callable=AsyncMock) as mock_place_retry:
            ...
            mock_circuit_breaker_system.record_critical_failure.assert_called_once_with("hyperliquid", "Symbol mapping failed during compensation for UNKNOWN")

    @pytest.mark.asyncio
    async def test_execute_opportunity_success_concurrent(self, execution_handler, sized_opportunity, mock_symbol_mapper, mock_hl_api, mock_bp_api, mock_circuit_breaker_system):
        ...
        long_order = Order(id="long1", symbol="BTC-PERP", side=OrderSide.BUY, type=OrderType.MARKET, quantity=long_qty, status=OrderStatus.FILLED, filled_quantity=long_qty, price=hl_ticker.price, avg_fill_price=hl_ticker.price)
        short_order = Order(id="short1", symbol="BTC_USDC", side=OrderSide.SELL, type=OrderType.MARKET, quantity=short_qty, status=OrderStatus.FILLED, filled_quantity=short_qty, price=bp_ticker.price, avg_fill_price=bp_ticker.price)
        ...
        with patch.object(execution_handler, '_place_order_with_retry', new_callable=AsyncMock) as mock_place_retry:
            ...
            def place_side_effect(*args, **kwargs):
                if kwargs.get('exchange_id') == "hyperliquid" and kwargs.get('symbol') == "BTC-PERP":
                    return long_order
            ...
            mock_place_retry.assert_any_call(
                client=mock_hl_api, exchange_id="hyperliquid", symbol="BTC-PERP", side=OrderSide.BUY,
                order_type=OrderType.MARKET, quantity=long_qty, price=None
            )
            mock_place_retry.assert_any_call(
                client=mock_bp_api, exchange_id="backpack", symbol="BTC_USDC", side=OrderSide.SELL,
                order_type=OrderType.MARKET, quantity=short_qty, price=None
            )
    ...
    async def test_execute_opportunity_compensation_needed(self, execution_handler, sized_opportunity, mock_symbol_mapper, mock_hl_api, mock_bp_api, mock_circuit_breaker_system):
        ...
        long_qty = (Decimal("1000") / hl_ticker.price).quantize(Decimal("1e-6"))
        long_order = Order(id="long1", symbol="BTC-PERP", side=OrderSide.BUY, type=OrderType.MARKET, quantity=long_qty, status=OrderStatus.FILLED, filled_quantity=long_qty, price=hl_ticker.price, avg_fill_price=hl_ticker.price)
        short_order_failure_exception = APIError("Short order failed", code=APIErrorCode.UNKNOWN, exchange_code="backpack")

        # Mock successful compensation order (placed on hyperliquid to close the long)
        comp_order = Order(id="comp1", symbol="BTC-PERP", side=OrderSide.SELL, type=OrderType.MARKET, quantity=long_qty, status=OrderStatus.FILLED)
        ...
        with patch.object(execution_handler, '_place_order_with_retry', new_callable=AsyncMock) as mock_place_retry:
            ...
            def place_side_effect(*args, **kwargs):
                ...
                print(f"Mock place_order call {call_count}: {exchange} {symbol} {side}") # Debug print

                if exchange == "hyperliquid" and symbol == "BTC-PERP" and side == OrderSide.BUY and call_count <= 2:
                    print("  Returning long_order")
                    return long_order
                elif exchange == "backpack" and symbol == "BTC_USDC" and side == OrderSide.SELL and call_count <= 2:
                    print("  Raising short order failure")
                    raise short_order_failure_exception
                elif exchange == "hyperliquid" and symbol == "BTC-PERP" and side == OrderSide.SELL: # Compensation call
            ...
            assert "Compensation successful." in execution.error_message # Check compensation message
            assert execution.long_order_id == "long1"
            ...
            mock_circuit_breaker_system.record_api_error.assert_any_call("backpack", f"Concurrent order placement failed: {short_order_failure_exception}")
