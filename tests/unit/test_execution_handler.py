...
from cyberdelta.core.execution_handler import ExecutionHandler, CircuitBreaker, ExecutionStatus, TradeExecution
...
class TestExecutionHandler:
    ...
    async def test_circuit_breaker_open_rejection(self, execution_handler, sized_opportunity):
        """Test that executions are rejected when circuit breaker is open."""
        # Open the circuit breaker
        execution_handler.circuit_breaker.open = True
    ...
    async def test_execute_opportunity(self, execution_handler, sized_opportunity, mock_exchange_api):
        ...
        with patch.object(execution_handler, '_place_order_with_retry', AsyncMock()) as mock_place_order, \
    ...
    def test_reset_circuit_breaker(self, execution_handler):
        ...
        execution_handler.circuit_breaker.failed_trades_count = 5
        execution_handler.circuit_breaker.consecutive_failures = 3
        execution_handler.circuit_breaker.total_loss = 50.0
        execution_handler.circuit_breaker.open = True
        ...
        assert execution_handler.circuit_breaker.failed_trades_count == 0
        assert execution_handler.circuit_breaker.consecutive_failures == 0
        assert execution_handler.circuit_breaker.total_loss == 0.0
        assert execution_handler.circuit_breaker.open is False 